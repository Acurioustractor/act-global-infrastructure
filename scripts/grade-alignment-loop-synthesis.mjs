...
async function tier23(text, slug, fm, _llmAvailable) {
  const prompt = buildTier23Prompt(text, slug, fm);
  const envModel = process.env.GRADE_SYNTHESIS_MODEL;
  const forceProvider = envModel ? providerFromModelName(envModel) : null;
  const raw = await trackedAgentCompletionWithFallback(prompt, 'grade-alignment-loop-synthesis', {
    task: 'generate',
    model: envModel || undefined,
    forceProvider,
    maxTokens: 2500, // bumped from 1500 for MiniMax <think> headroom
    operation: 'grade-synthesis-tier23',
