...
function normalizePost(post, accounts) {
  ...
  // A known account decides by name. The Facebook/Instagram fallback only covers posts whose
  // account GHL no longer returns (the retired Harvest Instagram), so another project's Meta
  // account can never land on the Harvest feed.
  const harvest = accounts.length
    ? accounts.some(isHarvestAccount)
  ...
  return {
    ...
    // displayDate is the scheduled time; only trust it once GHL says the post went out.
    published_at: post.publishedAt || (post.status === 'published' ? post.displayDate : null) || null,
