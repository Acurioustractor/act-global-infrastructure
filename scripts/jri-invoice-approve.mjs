#!/usr/bin/env node
/**
 * Approve (AUTHORISE) the Justice Reform Initiative photography invoice and
 * re-save the finalised PDF. Does NOT email the contact. Run with --apply.
 *   node scripts/jri-invoice-approve.mjs --apply
 */
...
const INVOICE_ID = '9288d804-c3c6-4c38-96c9-742a8debc659'; // INV-0333
...
async function getAccessToken() {
  ...
  await supabase.from('xero_tokens').upsert({ id: 'default', refresh_token: t.refresh_token, access_token: t.access_token, expires_at: expiresAt.toISOString(), updated_at: new Date().toISOString(), updated_by: 'jri-approve' }, { onConflict: 'id' });
...
async function main() {
  ...
  const outPath = path.join(os.homedir(), 'Downloads', `JRI-photography-invoice-${cur.InvoiceNumber}.pdf`);
