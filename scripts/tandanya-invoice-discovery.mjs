#!/usr/bin/env node
/**
 * READ-ONLY discovery for the Tandanya / CONTAINED invoice.
 * - Refreshes Xero token (same rotation as apply-ge-recode-to-xero.mjs)
 * - Shows connected org (name + ABN) and ALL available connections (entity choice)
 * - Searches for a Tandanya contact
 * - Lists revenue accounts + the GST-on-income tax rate
 * Makes NO writes to Xero.
 */
...
async function getAccessToken() {
  ...
  await supabase.from('xero_tokens').upsert({ id: 'default', refresh_token: t.refresh_token, access_token: t.access_token, expires_at: expiresAt.toISOString(), updated_at: new Date().toISOString(), updated_by: 'tandanya-discovery' }, { onConflict: 'id' });
...
async function main() {
  ...
  // All connections (which entities this token can reach)
  ...
  // Tandanya contact?
  const cRes = await (await fetch(`https://api.xero.com/api.xro/2.0/Contacts?where=${encodeURIComponent('Name.Contains("Tandanya")')}`, { headers: xh(token) })).json();
  console.log('\n=== CONTACT SEARCH "Tandanya" ===');
  if (!cRes.Contacts?.length) console.log('  (none found — will need to create)');
