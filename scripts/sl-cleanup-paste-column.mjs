...
const flag=v=>{const t=[];if(v.needs_ben)t.push('NEEDS YOUR CALL');if(v.receipt_status==='GAP_PLEASE_PROVIDE')t.push('receipt to provide — check Dext');if(v.receipt_status==='GMAIL_CANDIDATE')t.push('receipt likely in Gmail — confirm');if(v.receipt_status==='GMAIL_FOUND')t.push('receipt located — forwarding');if(v.receipt_status==='GMAIL_LEAD')t.push('booking confirmation found — folio to confirm');if(v.receipt_status==='DEXT_IN')t.push('receipt in Dext');if(v.receipt_status==='RECEIPT_VENDOR_MISMATCH')t.push('no matching receipt on file');if(v.verified===false)t.push('reviewer-corrected');return t.length?`[${t.join('; ')}] `:'';};
