...
supporters.forEach((s,i)=>{s.layer=i<5?'5':i<15?'15':i<50?'50':i<150?'150':null;});
...
const DATA=JSON.stringify({supporters,community:namedCon,unnamedCon,projects:projData,stats});
...
// cold cloud label
const cc=el('text',{x:cx,y:cy+bands.light+34,fill:'#46566b','text-anchor':'middle','font-size':11});cc.textContent='+ '+D.stats.cold+' cold / uncaptured (the periphery)';svg.appendChild(cc);
