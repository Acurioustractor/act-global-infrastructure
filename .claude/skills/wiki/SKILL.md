...
The narrative store at `wiki/narrative/<project>/` records what ACT has already said about
a project, one claim per file, so the next post builds on the last one. It has its own
subcommands (status, draft, log, lint, conflicts). When the user asks for anything
narrative, read `references/narrative.md` in this skill before acting.
