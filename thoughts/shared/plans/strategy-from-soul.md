...
9. ✅ Add the one extra line to the weekly digest. (Done in commit `583df7c` "feat(cron): weekly digest carries four-lanes + soul check". `scripts/weekly-reconciliation.mjs:418-453` runs `tag-lanes.mjs` + `four-lanes-snapshot.mjs` then prints `*Four lanes (Q FY26):* To Us $X · To Down $Y · To Grow $Z · To Others $W` + `LCAA: L X% · C Y% · A Z% · Art W%` + `🌱 *Soul check:* lane most behind = *<lane>* ($X). <question>` into the Monday 8am Telegram digest. Soul-check questions are encoded per lane: To Us "When did Ben and Nic last get paid by the entity that earns the money?", To Down "What old liability is unblocked?", To Grow "Which project most needs the next dollar?", To Others "Which community partner hasn't had a fellowship or anchor payment yet?")
