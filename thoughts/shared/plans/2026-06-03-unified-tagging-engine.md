...
- [x] **`/finance/tagging` rebuilt → "Retag & reconcile" cockpit** — coverage bars (make-sense) + Xero cross-link to `/finance/tagger-v2` · conflicts per-row (lazy `ACT-CA` default-accept; specific-vs-specific flagged **"your call"** + default-keep; per-row override dropdown) · auto-fills grouped-by-code, collapsible, pre-checked, uncheck outliers · **no-match assignable** (opps grouped by pipeline with "assign all → code" per group + per-row override; subs with category; default leave) · sticky **Apply N** bar + post-apply **Undo last apply**. Sweep enriched to carry `pipeline` (opps) / `category` (subs) on items for the grouping. Verified rendering + live use, 0 console errors.
...
- [x] **No-match assigned by Ben in the cockpit (2026-06-03):** **The Shop pipeline ×28 → ACT-HV** (the Witta farm shop — Maleny markets/makers); **6 subs** → X·Amazon Prime·LinkedIn·Garmin = **ACT-IN** (infra), SumUp = **ACT-HV** (POS), Starlink = **ACT-GD** (mainly Goods, remote satellite). **Final: opps 94% (722/769), subs 100% (68/68), conflicts 0.** **LEFT UNCODED ON PURPOSE — 47 opps:** Grants (27, prospective funding — code when won, not now) + Universal Inquiry (20, mostly test/$0 junk). 94% is "done" for real opps; the rest is correctly-uncoded noise.
