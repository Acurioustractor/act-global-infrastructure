---
title: Goods on Country, Minderoo envelope (working draft)
status: Draft for Nic review
date: 2026-05-06
author: Ben (with ACT voice)
audience: Lucy Stronach, Minderoo (lstronach@minderoo.org)
lands: Mid-May 2026 (~3 weeks after JusticeHub envelope of 1 May)
related: thoughts/shared/research/goods-minderoo-pitch/pitch-architecture.md
---

# Goods on Country, Minderoo envelope (working draft)

> This is the operational draft of the pack. Voice-loaded, evidence-current, blocked only on the items in §7 (the ask itself, pending Nic's call on QBE matched-side structure). Cover, executive summary, problem, work, and ecosystem sections are submission-ready for Nic's review.

---

## Entity reality (updated 2026-05-06)

Earlier architecture spoke of "ACT Foundation" / "ACT Ventures." Those names are not legal entities. The truth on the ground:

- **A Curious Tractor Pty Ltd** (ACN 697 347 676), trading entity. Registered 24 April 2026. Knight + Marchesi family trusts 50/50. Goods on Country sits here. Cutover from Nic's sole trader 30 June 2026.
- **A Kind Tractor Ltd** (ACN 669 029 341), charity, dormant, NOT DGR-endorsed.

**Implication for Minderoo:** Goods is housed in a Pty Ltd, not a DGR. That makes the ask a Program-Related Investment (PRI) on the matched side, not a grant on Minderoo's distribution side. This is consistent with the QBE Catalysing Impact 2026 cohort design, which explicitly stacks catalytic grant with matched investment. Lock with Nic + Standard Ledger before lodging.

---

## §1. Cover

**The basket leaves country. The hand at market is paid. Goods is the route between.**

Three lines. ACT mark. Pretext type, black on cream.

Operational reading (deck-side, second slide): Goods on Country is the demand-side layer for First Nations employment parity.

---

## §2. Executive summary

Minderoo built the supply side. Generation One. Dream Venture. Backing Black Business. First Australians Capital. The basket is full. The hand that fills it is First Nations. The buyer who carries it home is not.

Indigenous Procurement Policy reaches Commonwealth agencies. Mid-market buyers, the businesses that move the bulk of Australian trade, have no equivalent layer. The supply side waits for a demand side that has not arrived.

Goods on Country is that layer.

We curate anchor buyers with verified spend. We vet and onboard First Nations suppliers. We facilitate contracts. We measure parity outcomes against the Indigenous Employment Index. We acquit, monthly, against Xero.

In FY25-26 Goods has invoiced **$1.35 million** through real buyer contracts. **$621,000 has settled.** The open pipeline across 128 opportunities sits at **$16.3 million.** The operational layer that runs underneath this, the Buyer Pipeline, Demand Register, Procurement Analyst, Funder Cadence agent, Invoice Drift Detector, Reconciler, is in production now, not on a roadmap.

QBE Australia announced ACT in the 2026 Catalysing Impact cohort on 23 March. Stage 2 grants, AU$150,000 to AU$400,000, are designed to draw matched investment. QBE writes that the 2025 cohort drew 2.7x.

We are inviting Minderoo to be the matched side. The dollar figure sets the stack.

---

## §3. The problem, anchored to CoLI24

The Cost of Late Intervention 2024 report, which Minderoo supported through the Front Project, costs the failure of participation at $22.3 billion nationally. The "Participating" precondition fails across eight of nine domains. NEET rates have climbed from 12.6 per cent to 13.4 per cent. The Northern Territory cost per head sits at $2,808, driven by remoteness and what the report names plainly as "levels of Indigeneity."

The report diagnoses the failure of participation. It does not prescribe the demand-side intervention.

That gap is where Goods sits. Indigenous Procurement Policy is one instrument, narrow to Commonwealth procurement. Outside Commonwealth, mid-market buyers have intent and no path. Supply Nation certifies. First Australians Capital invests. Dream Venture trains. Goods routes the demand into the certified, financed, capable enterprises these programs already produce.

When the basket moves from country to market, the participation precondition resolves through trade, not transfer. Forrest's frame, not welfare's.

---

## §4. The work

Goods runs a 12-stage Buyer Pipeline. Stages move from cold prospect through verification, contract drafting, onboarding, first order, repeat order, and sustained supply.

The Demand Register catalogues every buyer interaction with line of sight to dollar value and Indigenous spend percentage. It is queryable by region, sector, and product class.

Supplier vetting is in two parts: Supply Nation certification check, and an internal capability profile that maps each supplier to their delivery ceiling. We do not place a buyer with a supplier who cannot fulfil. We place buyers with suppliers we have already watched deliver.

Acquittal runs through Xero, line-item per contract, and reconciles weekly into the agent layer. The Invoice Drift Detector watches our wiki claims against live Xero state. The Procurement Analyst surfaces the top three weekly buyer touches. The Funder Cadence agent watches funder silence and drafts a progress note when the silence stretches. None of this is novel. All of it is operational.

The team:
- Ben Knight (founder, infrastructure)
- Nic Marchesi OAM (founder, partner relationships)
- [First Nations operational lead, named after Nic conversation]
- [First Nations governance advisor, named after Nic conversation]

---

## §5. The evidence

(numbers refreshed 2026-05-06 from live Supabase + Xero feeds)

| Signal | Current state |
|---|---|
| FY25-26 invoiced through Goods | **$1.35M** (30 invoices) |
| FY25-26 received | **$621K** |
| Open buyer pipeline | **$16.3M** across 128 opportunities |
| Receipt match rate (audit-ready) | 95.3 per cent |
| Agent layer in production | 5 agents, MiniMax-M2.7 backend, daily cadence |
| Reporting cadence | Monday morning chain refreshes 12 dashboard builds in Notion before 9:30 AEST |

Named anchor buyers and named suppliers belong in this section, in a one-page table. Pulling that list is a 30-minute exercise once Nic confirms which relationships he wants Lucy to see by name.

The JusticeHub envelope, which Lucy received 1 May, demonstrates the same anchor-portfolio method working in a different market. The two envelopes share architecture, not budget.

---

## §6. Ecosystem fit

Goods does not compete with the system Minderoo has built. Goods routes demand to it.

| Adjacent | Side of the system | Goods relationship |
|---|---|---|
| First Australians Capital | capital | we contract enterprises FAC has financed |
| Dream Venture / DreamSpark | capability | we contract enterprises Dream Venture has trained |
| Backing Black Business | capability + capital | we onboard graduates as suppliers |
| Supply Nation | certification | every Goods supplier carries Supply Nation certification |
| Murawin Consulting | evaluation | we propose Murawin as the independent parity evaluator |
| Blak Angels | deal flow | we accept supplier referrals from the Blak Angels network |

The measurement frame is the Indigenous Employment Index. Quarterly parity reporting maps Goods deliverables to IEI categories.

---

## §7. The ask (blocked, pending Nic)

**Held back until 10-minute call with Nic resolves three structural questions:**

1. Confirm the QBE matched-side: is the $400,000 a Catalysing Impact 2026 Stage 2 grant, or a separate bilateral?
2. Confirm the matched-side mechanism Minderoo is asked to underwrite: is it grant from Minderoo Foundation (DGR-side), or PRI from Minderoo Investments (equity / repayable debt to ACT Pty Ltd)?
3. Confirm the dollar figure and term:
   - **Match-mirror option:** $400,000 over 12 months. Symmetrical with QBE.
   - **Stack-step option:** $250,000 + $400,000 + $400,000 over 36 months. Same total, different drawdown.
   - **Anchor option:** $900,000 over 36 months. Larger than QBE, signals Minderoo as lead funder.

**Once Nic resolves the above, this section becomes:**
- Dollar figure (above options).
- Term.
- Deliverables: 8 quarterly reports against IEI parity categories. Annual independent evaluation by Murawin or equivalent. Named anchor cohort and supplier cohort, with attainment thresholds.
- Matched-side mechanic: precise stack with QBE's commitment, what releases at each tranche.
- Governance: ACT Pty Ltd board, First Nations advisory body, named reporters to Minderoo.

**Ben + Nic to decide before 12 May.**

---

## §8. Appendices (build out after §7 resolved)

- Full budget, line item.
- ACT Pty Ltd governance and First Nations advisory.
- Evaluation frame mapped to IEI categories.
- Data + acquittal protocol (one page on Xero, GHL, agent layer).

---

## The envelope itself (draft email to Lucy, ~150 words)

Subject: **Goods on Country, the demand-side piece**

Lucy,

The basket leaves country. The hand at market is paid. Goods is the route between.

Three weeks ago you read the JusticeHub envelope. Different market, same architecture.

This is the second envelope. Goods on Country curates anchor buyers, vets First Nations suppliers, facilitates contracts, measures parity. In FY25-26 Goods has invoiced $1.35 million through real trade. The open pipeline sits at $16.3 million. The agent layer that runs underneath is in production.

QBE has placed ACT in the 2026 Catalysing Impact cohort. The Stage 2 grant is designed to draw matched investment. We are inviting Minderoo to be the matched side.

Pack attached. Ben + Nic ready for a 30-minute conversation any time in the next three weeks.

Best,
Ben Knight
A Curious Tractor

---

## Voice + style audit (before sending)

Run through:
- [ ] No em-dashes. Use commas and full stops.
- [ ] First Nations, not Indigenous (except in Indigenous Procurement Policy and Indigenous Employment Index, which are proper nouns).
- [ ] Employment parity, catalytic, systems-level, financial inclusion, self-determination.
- [ ] Never: welfare, closing the gap, Voice (unloaded), empowerment, reconciliation as primary frame.
- [ ] Curtis method on every weighted sentence: name the room, name the body, load the abstract noun, stop the line.
- [ ] No "delve, crucial, pivotal, tapestry, leveraging, fostering, in the heart of."
- [ ] No "not just X but Y." No rule-of-three adjectives. No "-ing" significance tails.
- [ ] Australian spelling.
- [ ] Sentence-case headings.
- [ ] Plainness test: a fourteen-year-old in Doomadgee can follow it.

---

## What's next on this draft

1. **Nic, 10-min call.** Confirm:
   - QBE Stage 2 status and the $400K figure.
   - PRI vs grant on Minderoo side.
   - Dollar / term option (match, step, or anchor).
   - First Nations operational lead and governance advisor names.

2. **Refresh §5 evidence table** with named anchor buyers (Nic to greenlight which to disclose).

3. **Build §7 ask** once Nic resolves the structural questions.

4. **Sweep against §"Voice + style audit"** before sending.

5. **Land on Lucy's desk by 16 May 2026** to give her three weeks before any panel review.
