...
date: 2026-04-24
sources_queried:
  - xero_invoices ACCREC status!=VOIDED/DELETED income_type=grant or null+funder-name (24 rows)
  - ghl_contacts tag funder or %funder% prefix (~41 rows)
  - communications_history joined against funder-tagged ghl_ids (28 rows)
  - wiki/narrative/funders.json (14 entries, file last updated 2026-04-09)
  - thoughts/shared/{plans,drafts}/** funder-name grep (~30 files)
---
...
1. **Call Snow Foundation within 2 weeks.** Confirm INV-0321 payment + notify of Pty transition. (migration checklist §2). Owner: Ben. Deadline: 2026-05-08.
2. **Decide INV-0314 Centrecorp with Nic.** 10 minutes. (memory item, 4+ sessions). Owner: Ben + Nic. Deadline: 2026-04-29.
3. **Decide Rotary eClub INV-0222 with Nic.** Chase or write off. (new surface from this pass). Owner: Ben + Nic. Deadline: 2026-04-29.
4. **Update `wiki/narrative/funders.json`** with the seven absent funders. Specific keys to add: `funders["centrecorp-foundation"]`, `funders["rotary-eclub-outback-australia"]`, `funders["vincent-fairfax-family-foundation"]`, `funders["social-impact-hub-foundation"]`, `funders["state-of-qld-dfsdscs"]`, `funders["streetsmart-australia"]`, `funders["westpac-scholars-trust"]`. Fix `funders["snow-foundation"].stage` set from `warm` to `active-partner`. Fix `funders["paul-ramsay-foundation"].stage` set from `cold` to `warm`. Validate `funders["june-canavan-foundation"]` ($60K active-partner) against actual Xero history. Owner: Ben. Deadline: 2026-04-30.
5. **Decide on the 4 silent 107-day contacts** in `ghl_contacts` filtered by `last_comm < 2026-01-08 AND tag = funder`: The Bryan Foundation (Matthew Cox, ghl_contact_id matched), The Funding Network (Kristen Lark), AMP Foundation Tomorrow Makers, Queensland Gives. Re-engage or drop. Owner: Ben. Deadline: 2026-05-01.
6. **Verify Minderoo ask figure.** Wiki says $2.9M (`funders["minderoo"].ask_amount_aud`). Goods pitch says $900K (`thoughts/shared/drafts/minderoo-goods-pitch-2026-05.md` §7). Envelope is a different thing again. Canonicalise one number per ask in funders.json. Owner: Ben + Nic. Deadline: 2026-05-12 (before Minderoo deadline 2026-05-15).
