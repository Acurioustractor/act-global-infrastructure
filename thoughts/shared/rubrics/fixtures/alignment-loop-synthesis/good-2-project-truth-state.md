...
date: 2026-04-24
sources_queried:
  - config/project-codes.json (74 codes parsed)
  - wiki/projects/**/*.md (slug + title presence per code)
  - xero_invoices.project_code activity in FY26 (per-code bill counts and totals)
  - codebase grep for project_code references (scripts/, apps/command-center/src/)
  - xero tracking categories per code (canonical map)
---
...
1. **Every active or ideation project scores ≥2/4** (against config/project-codes.json scoring rubric in this synthesis §Methodology). The 4 projects scoring 1/4 are all archived admin artefacts: `ACT-APO` Overview (config row 71), `ACT-AMT` Migration Test (config row 72), `ACT-XYZ` placeholder (row 73), `ACT-TST` legacy test (row 74).
2. **28 of 74 projects (38%) are fully aligned at 4/4** per the per-code scoring table below. Examples on the 4/4 list: `ACT-EL` Empathy Ledger (xero_invoices.project_code=ACT-EL: 8 bills, $1,304 FY26), `ACT-GD` Goods (154 bills, $247,837 FY26), `ACT-CG` CivicGraph (parented under ACT-IN: 516 bills, $224,144 FY26 per Decision 4), `ACT-JH` JusticeHub (3 bills, $701 FY26).
3. **The authoring backlog is small and specific.** Three active projects have DB activity but no dedicated wiki article: `ACT-IN` (internal ops - arguably fine), `ACT-PS` PICC On Country Photo Studio (real gap - $9K paid per xero_invoices.project_code=ACT-PS, 47 code refs, no article), and `ACT-SM` SMART (covered by `wiki/projects/smart-recovery/smart-recovery.md` but config lacks `canonical_slug`, so automated cross-ref misses it).
4. **Methodology caveat: 40+ codes in `project-codes.json` have no `canonical_slug`.** Specific examples: `ACT-CT` ConFit, `ACT-RT` Redtape, `ACT-TR` Treacher, `ACT-FO` Fishers, `ACT-BV` BCV (5 of the 40 listed in §Score distribution drilldown). The wiki has articles the scoring can't find because the config doesn't say which article covers which code. Fixing this is what enables Phase-1 automation of this synthesis.
...
1. **Add `canonical_slug` fields** to `config/project-codes.json` for the 40+ codes missing them (full list in §Score distribution drilldown). Unblocks Phase-1 automation of this synthesis. Owner: Ben. Deadline: 2026-05-15.
2. **Remove `ACT-APO`, `ACT-AMT`** keys from `config/project-codes.json` (rows 71-72). Self-described non-projects. Owner: Ben. Deadline: 2026-04-30.
3. **Decide on `ACT-EFI`, `ACT-GCC`** keys in `config/project-codes.json`. Archived + no traces. Outcome options: (a) remove keys, (b) annotate `archived: true` and keep as historical markers. Owner: Ben + Nic. Deadline: 2026-04-29.

### Wiki authoring (one real gap)
4. **Draft `wiki/projects/picc/picc-on-country-photo-studio.md`** - active project with $9K paid (xero_invoices.project_code=ACT-PS) and 47 code refs but no wiki article. Owner: Ben. Deadline: 2026-05-12.

### Financial tracking hygiene
5. **Decide CAMPFIRE / ConFit Pathways Xero routing** with Nic. Are their expenses flowing through ACT? If yes: tag in Xero under `ACT-CM` and `ACT-CT` per `config/project-codes.json` keys. If no: document arrangement in `wiki/projects/campfire/campfire.md` §Financials and `wiki/projects/confit-pathways/confit-pathways.md` §Financials. Owner: Ben + Nic. Deadline: 2026-05-08.

### Phase-1 automation readiness
6. **Turn this synthesis into `scripts/synthesize-project-truth-state.mjs`** once the config hygiene is done. The queries and scoring logic in this doc directly translate. Owner: Ben. Deadline: 2026-06-01 (after action #1 closes).
