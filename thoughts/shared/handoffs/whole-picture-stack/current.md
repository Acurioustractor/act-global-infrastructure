...
[->] **LEFT (still open):**
1. ⚠️ **PRE-DEPARTURE BLOCKER — Mac sleep.** `pmset` system `sleep = 1` (minute). Once Ben leaves and the apps holding it awake close, the Mac sleeps and **PM2 cron stops firing for the whole trip**. Fix before 27 Jun: `sudo pmset -c sleep 0` (never sleep on AC) — needs Ben's sudo. This is the real pre-departure cron drill action.
2. **`TELEGRAM_CHAT_ID_NIC` still unset** — without it Nic gets no founders-kit / Monday-card Telegram (Ben's still send). Not blocking; paste the value into `.env.local` when handy.
