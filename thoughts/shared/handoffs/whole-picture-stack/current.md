...
**Updated:** 2026-06-16 PM (v1.5 COMPLETE build-side; **PM2 cron set TRIMMED 133→10** for the trip — all v1.5 crons kept, see [done 2026-06-16 PM] below; **Nic-Telegram item CLOSED** — Nic has no Telegram; only Mac-sleep + founder gate-flips remain — see LEFT)
...
[decided 2026-06-16] **GCal founders'-session event → Ben's, not mine.** No session time-of-day exists in any plan; Ben will set up the recurring 1st-Tue (from 7 Jul 2026) invite himself. Dropped from my scope.
[decided 2026-06-16] **Cron host for 27 Jun–7 Aug = LOCAL MAC (stays on).** No cloud migration. PM2 boot-resurrect is configured (`~/Library/LaunchAgents/pm2.benknight.plist`) + dump current with the 3 new crons.

[done 2026-06-16 PM] **PM2 cron set TRIMMED 133→10 (pre-trip de-bloat; Ben: "133 cron jobs seems dumb").** Paused 122 jobs via `pm2 delete` + `pm2 save` (Ben ran it — auto-mode classifier correctly blocked the agent from mass-deleting its own selection). **KEPT (10):** xero-sync · xero-bank-balances · two-account-cash · rd-basis · whole-picture · eofy-burndown · monday-card · founders-session-prep · act-frontend (live server) · telegram-queue-drain (quiet-hours glue, protects the founders/EOFY pushes). The entire v1.5 chain survived intact. All **7 foreign processes** (GrantScope `orchestrator`, the `empathy-ledger*` set, `ten-years`) untouched — pause list filtered to THIS config's app names only. Verified runtime = exactly 10 keepers + 7 foreign, nothing collateral. **Fully reversible** — `ecosystem.config.cjs` still defines all 133; re-arm one = `pm2 start ecosystem.config.cjs --only <name> && pm2 save`, everything = drop `--only`. Restore record + full pause list: `thoughts/shared/handoffs/cron-trim-2026-06-16.md`. **The empirical cull:** whatever Ben never re-arms in August was always dead weight — delete from config then.

[->] **LEFT (still open):**
1. ⚠️ **PRE-DEPARTURE BLOCKER — Mac sleep.** `pmset` system `sleep = 1` (minute). Once Ben leaves and the apps holding it awake close, the Mac sleeps and **PM2 cron stops firing for the whole trip**. Fix before 27 Jun: `sudo pmset -c sleep 0` (never sleep on AC) — needs Ben's sudo. Still pending as of 2026-06-16 PM. ⚠️ **HOST IS A MacBook Pro** (confirmed from shell prompt) → `sleep 0` ALONE is insufficient if the lid closes — clamshell sleeps regardless. **Required setup: lid OPEN + on power + ventilated** (simplest, safe); only if the lid must close would you also need `sudo pmset -c disablesleep 1` (kills thermal sleep too — avoid). Verify after: `pmset -g custom | grep -i ' sleep'` → `sleep 0`.
2. ~~`TELEGRAM_CHAT_ID_NIC`~~ **CLOSED 2026-06-16 — Nic has no Telegram.** Nothing to set; `build-monday-card.mjs:706` guards the Nic send on the env var, so unset = clean skip (Ben still receives his). ⚠️ **PRODUCT GAP surfaced:** Nic's only channel for the money read is now PULL-only (the standing Whole Picture Notion page + the card HTML) — no push. Confirm Nic actually opens that page for the founders' session, else close the delivery gap (email the card / Ben relays).
3. **Founders to decide (not mine):** N3 one-money-truth → flip `WHOLE_PICTURE_MONEY_CANON=1` · the #8815 reconciliation that tightens the cash band · the R&D records cure → `RD_BASIS_RECORDS_CURED=1`.
