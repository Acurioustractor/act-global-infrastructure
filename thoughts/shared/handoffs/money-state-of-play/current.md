...
**NEXT — needs Ben's call (all Tier-3 / his judgment):**
1. **Plan 1 Tier-3 (optional):** capture Xero `BankAccountType` into `xero_bank_accounts.bank_account_type` (migration + sync edit + backfill) so new cutover accounts auto-class. Investigation CONFIRMED Xero exposes it (Visa=CREDITCARD). Behaviorally identical today (registry covers #8815) — cutover-proofing only.
2. **Plan 2 (ingest deposit account):** the ACT Everyday deposit txns ARE in `xero_transactions` (106 FY rows) but `bank_statement_lines` is 100% NAB Visa. Choose source: derive from xero_transactions vs ingest a raw ACT Everyday statement export (needs the file + a new parser). Depends on Plan 1's deposit class.
3. **Tagging queue (Ben's live workbench work):** real untagged = 0 now. Backlog is **ACT-IN reassignment** — 897 xero_transactions + 633 invoices tagged to the ACT-IN catch-all need real project codes. His judgment, Tier-3 writes via `/finance/workbench` (Review ACT-IN card).
