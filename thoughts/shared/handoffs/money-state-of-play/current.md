...
**Updated:** 2026-06-01 (PM-6) — ALL remaining finance skills aligned to the workbench + money-guards (the planned NEXT, now done & pushed). Also fixed two real bugs found en route: `preflight`'s inverted Supabase refs (EL v2 = `yvnuayzslukamizrlhwb`, `uaxhjzqrdotoahjnxmbj` = media-only storage — ground-truthed vs codebase) and the `act-money-brain` R&D-refund SQL that had **no DELETED filter** (the ~$32K-inflation class of bug). Live command center on :3002 — `/finance/workbench` is the tagging surface.
**Read first:** `.claude/skills/tag-transactions/SKILL.md` (the TEMPLATE) · `apps/command-center/src/lib/finance/workbench.ts` · memory `command-center-finance-truth.md`.
**HEAD `33748fc`** on `wip/opus-4-8-prompting-2026-05-31`, SYNCED (pushed). Plugin repo `act-claude-plugins` `42e270b` pushed to master + mirrored to installed `~/.claude/plugins/` (money-brain guards live now).
**NEXT (do in a fresh context):** the 2 systemic plans (`thoughts/shared/plans/2026-06-02-account-class-credit-card-detection.md`, `2026-06-02-ingest-deposit-account-bank-lines.md`) + Ben's live tagging queue on `/finance/workbench` (Project gaps: 14 invoices + 4 bank income lines; Review ACT-IN: 2,599 catch-all; un-tag 4 transfers wrongly tagged to a project).
