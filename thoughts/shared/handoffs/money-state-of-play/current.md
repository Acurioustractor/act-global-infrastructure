...
**Updated:** 2026-06-01 (PM-7) — Plan 1 (account-class) Tier-1 SHIPPED + a NEW bug found & fixed while working the tagging queue: **invoice reads excluded DELETED but not VOIDED**. The "14 untagged invoices" tagging item was **100% voided phantoms → real untagged invoices = 0**. Canonical $ surfaces were already safe (allow-lists); leak was workbench-queue-only. Both commits are money-semantics → a clean-context review **CONFIRMED** both (5/5 standards, 7/7 tests, no bugs; independently verified the canonical $ surfaces use allow-lists so totals/R&D basis are untouched).
**Read first:** memory `command-center-finance-truth.md` (NEW: VOIDED≠DELETED bullet) · `apps/command-center/src/lib/finance/workbench.ts` (`accountClass`, `isVoidStatus`) · the 2 plan files below.
**HEAD `1f43880`** on `wip/opus-4-8-prompting-2026-05-31`, LOCAL (commits c7bc7e0 account-class + 1f43880 VOIDED — review CONFIRMED, ready to push on Ben's go).
**NEXT (PRIMED for a fresh context — start here):**
1. **▶ Plan 2 — derive ACT Everyday deposit bank lines from `xero_transactions`.** Approach chosen + **fully de-risked with a turnkey implementation spec** in `thoughts/shared/plans/2026-06-02-ingest-deposit-account-bank-lines.md` (read the "Implementation spec" section — it has the resolved double-count analysis, the exact field map, NOT NULL cols, the `onConflict` key, and the tracer-bullet steps). Derive ONLY 122 rows (65 SPEND + 57 RECEIVE), SKIP the 100 transfers. **Tracer-bullet ONE row → verify no double-count → bulk with Ben's explicit go (Tier-3 write).** Plan 1's deposit class already works for this account.
2. **Plan 1 Tier-3 — DEFERRED** (Ben). Capture Xero `BankAccountType` into the DB at the 30 Jun cutover (auto-classes new NAB accounts). Behaviorally identical today.
3. **Tagging backlog (Ben's live workbench work):** real untagged = 0. Backlog is **ACT-IN reassignment** — 897 xero_transactions + 633 invoices in the catch-all need real project codes. His judgment, Tier-3 writes via `/finance/workbench` (Review ACT-IN card).
