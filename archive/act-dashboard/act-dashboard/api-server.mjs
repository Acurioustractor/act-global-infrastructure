...
app.get('/api/connectors', async (req, res) => {
  try {
    // Connector definitions with fallback env var names (any one match = configured)
    const connectors = [
      { name: 'Supabase Main', type: 'database', required: [['SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_URL', 'SUPABASE_SHARED_URL'], ['SUPABASE_SERVICE_ROLE_KEY', 'SUPABASE_SHARED_SERVICE_ROLE_KEY']] },
      { name: 'Supabase EL', type: 'database', required: [['EL_SUPABASE_URL'], ['EL_SUPABASE_SERVICE_ROLE_KEY']] },
      { name: 'Notion', type: 'api', required: [['NOTION_TOKEN', 'NOTION_API_KEY']] },
      { name: 'GoHighLevel', type: 'crm', required: [['GHL_API_KEY'], ['GHL_LOCATION_ID']] },
      { name: 'Gmail/Calendar', type: 'oauth', required: [['GOOGLE_SERVICE_ACCOUNT_KEY', 'GOOGLE_CLIENT_ID']] },
      ...
      { name: 'Vercel', type: 'hosting', required: [['VERCEL_TOKEN']] }
    ];

    // Check which connectors have their env vars configured
    // Each required item is an array of fallback var names - any one match = satisfied
    const result = connectors.map(c => {
      const missingGroups = [];
      const configured = c.required.every(varGroup => {
        // varGroup is an array of fallback variable names
        const hasAny = varGroup.some(key => !!process.env[key]);
        if (!hasAny) missingGroups.push(varGroup[0]); // Report first name as missing
        return hasAny;
      });
      return {
        ...c,
        status: configured ? 'configured' : 'missing',
        missingVars: missingGroups
      };
    });
...
app.get('/api/relationships/overdue', async (req, res) => {
  try {
    ...
    // Only show contacts that HAVE been contacted before (not never-contacted)
    const { data: overdue, error } = await supabase
      ...
      .select(`
        ...
        total_touchpoints
      `)
      .not('last_contact_at', 'is', null)  // Must have been contacted before
...
// Get relationships needing attention (contacts we've engaged with but are now overdue)
app.get('/api/relationships/attention', async (req, res) => {
  try {
    const { limit = 20 } = req.query;

    // Get contacts that HAVE been contacted (not never-contacted) but are overdue
    const { data: attention, error } = await supabase
      ...
      `)
      .not('last_contact_at', 'is', null)  // Must have been contacted before
      .gte('days_since_contact', 30)       // Overdue (30+ days)
      .order('days_since_contact', { ascending: false })  // Most overdue first
      .limit(parseInt(limit));
...
// Real financial data from Xero tables in Supabase
async function getRealFinancialData() {
  try {
    // Get receivables (ACCREC) - invoices where we're owed money
    const { data: receivables } = await supabase
      .from('xero_invoices')
      .select('total, amount_due, amount_paid')
      .eq('type', 'ACCREC')
      .in('status', ['AUTHORISED', 'SUBMITTED']);

    // Get payables (ACCPAY) - invoices we owe
    ...
    const { data: transactions } = await supabase
      ...
      .select('id, date, contact_name, total, type, line_items')
      .order('date', { ascending: false })
      .limit(10);

    // Get this month's invoices for monthly summary
    ...
    // Calculate totals
    const totalReceivable = receivables?.reduce((sum, inv) => sum + Number(inv.amount_due || 0), 0) || 0;
    const totalPayable = payables?.reduce((sum, inv) => sum + Number(inv.amount_due || 0), 0) || 0;
    const netPosition = totalReceivable - totalPayable;

    const monthlyRevenue = monthlyInvoices?.filter(i => i.type === 'ACCREC').reduce((sum, i) => sum + Number(i.total || 0), 0) || 0;
    const monthlyExpenses = monthlyInvoices?.filter(i => i.type === 'ACCPAY').reduce((sum, i) => sum + Number(i.total || 0), 0) || 0;

    // Format transactions for response
    const recentTransactions = (transactions || []).map(tx => ({
      id: tx.id,
      date: tx.date,
      description: tx.contact_name || (tx.line_items?.[0]?.description) || 'Transaction',
      amount: tx.type === 'SPEND' ? -Math.abs(tx.total) : Math.abs(tx.total),
      category: tx.line_items?.[0]?.accountCode || 'Uncategorized'
    }));
    ...
    return {
      cashPosition: { net: 0, receivable: 0, payable: 0 },
      recentTransactions: [],
      monthlySummary: { revenue: 0, expenses: 0, net: 0 },
      lastUpdated: new Date().toISOString(),
      source: 'error',
      error: error.message
    };
...
// Real bookkeeping data from Supabase
async function getRealBookkeepingData() {
  try {
    // Get overdue invoices (past due date, still have amount_due > 0)
    ...
    // Get pending receipts count (only Oct 1, 2025 onwards)
    ...
    // Get this week's reconciliation progress
    ...
    // Calculate checklist progress (based on week's completion rate)
    const completed = weeklyProgress?.resolved_count || 0;
    const total = weeklyProgress?.total_pending || 0;
    const percentage = total > 0 ? Math.round((completed / total) * 100) : 100;

    // Calculate BAS due date (next quarter end + 28 days)
    const now = new Date();
    const quarter = Math.floor(now.getMonth() / 3);
    const quarterEnd = new Date(now.getFullYear(), (quarter + 1) * 3, 0);
    const basDue = new Date(quarterEnd);
    basDue.setDate(basDue.getDate() + 28);

    // Calculate GST owed (rough: 10% of receivables)
    const { data: paidInvoices } = await supabase
      .from('xero_invoices')
      .select('total')
      .eq('type', 'ACCREC')
      .eq('status', 'PAID')
      .gte('date', new Date(now.getFullYear(), quarter * 3, 1).toISOString().split('T')[0]);

    const quarterRevenue = paidInvoices?.reduce((sum, i) => sum + Number(i.total || 0), 0) || 0;
    const gstOwed = Math.round(quarterRevenue * 0.1);
    ...
    return {
      ...
      overdueInvoices: {
        ...
        total: overdueInvoices?.reduce((sum, i) => sum + Number(i.amount_due || 0), 0) || 0,
        invoices: overdueInvoices || []
      },
      pendingReceipts: {
        count: pendingReceiptsCount || 0
      },
      nextBASDue: basDue.toISOString().split('T')[0],
      gstOwed,
      source: 'supabase'
    };
  } catch (error) {
    console.error('Error fetching real bookkeeping data:', error);
    return {
...
app.get('/api/financial/summary', async (req, res) => {
  try {
    const financialData = await getRealFinancialData();
    res.json({
      success: true,
      ...financialData
    });
...
app.get('/api/financial/transactions', async (req, res) => {
  try {
    const { limit = 20 } = req.query;
    const data = await getRealFinancialData();
    res.json({
      success: true,
...
app.get('/api/bookkeeping/progress', async (req, res) => {
  try {
    const bookkeepingData = await getRealBookkeepingData();
    res.json({
      success: true,
      ...bookkeepingData
    });
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SUBSCRIPTIONS ENDPOINTS (Real Data)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

// Mock subscriptions data (fallback until migration applied)
function getMockSubscriptionsData() {
  return [
    { id: 'sub_1', name: 'Claude Pro', provider: 'Anthropic', cost_per_cycle: 32, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-15', category: 'ai', project_codes: ['ACT-AI'] },
    { id: 'sub_2', name: 'GitHub Copilot', provider: 'GitHub', cost_per_cycle: 16, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-01', category: 'development', project_codes: [] },
    { id: 'sub_3', name: 'Supabase Pro', provider: 'Supabase', cost_per_cycle: 40, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-10', category: 'infrastructure', project_codes: ['ACT-JH', 'ACT-EL'] },
    { id: 'sub_4', name: 'Vercel Pro', provider: 'Vercel', cost_per_cycle: 32, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-05', category: 'infrastructure', project_codes: [] },
    { id: 'sub_5', name: 'Xero', provider: 'Xero', cost_per_cycle: 85, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-01', category: 'finance', project_codes: ['ACT'] },
    { id: 'sub_6', name: 'Notion Team', provider: 'Notion', cost_per_cycle: 190, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-08', category: 'operations', project_codes: ['ACT'] },
    { id: 'sub_7', name: 'Cursor Pro', provider: 'Cursor', cost_per_cycle: 32, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-21', category: 'development', project_codes: [] },
    { id: 'sub_8', name: 'Dext Prepare', provider: 'Dext', cost_per_cycle: 45, currency: 'AUD', billing_cycle: 'monthly', status: 'active', renewal_date: '2026-02-05', category: 'finance', project_codes: ['ACT'] },
  ];
}
...
app.get('/api/subscriptions', async (req, res) => {
  try {
    ...
    // Use account_status column (not status)
    if (status && status !== 'all') {
      if (status === 'active') {
        // Include active + pending_migration as "active" for display
        query = query.in('account_status', ['active', 'pending_migration']);
      } else if (status === 'cancelled') {
        query = query.eq('account_status', 'cancelled');
      } else {
    ...
    const finalSubs = (subscriptions || []).map(sub => ({
      ...
      name: sub.vendor_name || sub.name,
      provider: sub.vendor_name || sub.provider,
      cost_per_cycle: parseFloat(sub.amount) || 0,
      status: sub.account_status || sub.status,
      renewal_date: sub.renewal_date || sub.next_renewal,
    }));

    // Calculate totals by currency
    const totals = { AUD: 0, USD: 0 };
    finalSubs.forEach(sub => {
      const amount = parseFloat(sub.amount) || 0;
      const monthlyAmount = sub.billing_cycle === 'annual' ? amount / 12 :
                           sub.billing_cycle === 'quarterly' ? amount / 3 :
                           amount;
      const currency = sub.currency || 'AUD';
...
// Get subscriptions summary for dashboard widget
app.get('/api/subscriptions/summary', async (req, res) => {
  try {
    ...
    if (error) {
      console.error('Subscriptions summary error:', error.message);
      return res.status(500).json({ success: false, error: error.message });
    }

    let finalSubs = subscriptions || [];
    ...
    finalSubs.forEach(sub => {
      ...
      if (!sub.project_codes || sub.project_codes.length === 0) {
        unassignedCount++;
      }

      if (sub.renewal_date && new Date(sub.renewal_date) <= twoWeeksFromNow) {
        dueSoonCount++;
      }
    });

    // Top 5 subscriptions by cost
    const topSubscriptions = [...finalSubs]
      .sort((a, b) => (parseFloat(b.amount) || 0) - (parseFloat(a.amount) || 0))
      .slice(0, 5)
      .map(s => ({
        id: s.id,
        name: s.vendor_name || s.name,
        provider: s.vendor_name || s.provider,
    ...
    res.json({
      success: true,
      total_monthly_aud: Math.round(totals.AUD || 0),
      total_monthly_usd: Math.round(totals.USD || 0),
      total_yearly_aud: Math.round((totals.AUD || 0) * 12),
      total_yearly_usd: Math.round((totals.USD || 0) * 12),
      count: finalSubs.length,
    ...
    const mockSubs = getMockSubscriptionsData().filter(s => s.status === 'active');
    res.json({
      success: true,
      total_monthly_aud: 130,
      total_monthly_usd: 215,
      total_yearly_aud: 1560,
      total_yearly_usd: 2580,
      count: mockSubs.length,
      unassigned: 3,
      dueSoon: 2,
      byCategory: { ai: 1, development: 2, infrastructure: 2, finance: 2, operations: 1 },
      topSubscriptions: mockSubs.slice(0, 5).map(s => ({
        id: s.id,
        name: s.name,
        provider: s.provider,
        amount: s.cost_per_cycle,
        currency: s.currency,
        billing_cycle: s.billing_cycle,
        project_codes: s.project_codes
      }))
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// SUBSCRIPTION DISCOVERY (must be BEFORE :id route)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

// GET /api/subscriptions/discover - Get discovery status and alerts
...
// GET /api/subscriptions/alerts - Get subscription alerts only
app.get('/api/subscriptions/alerts', async (req, res) => {
  try {
    ...
    res.json({ success: false, alerts: [], error: error.message });
  }
});

// GET /api/subscriptions/pending - Get pending subscription discoveries
app.get('/api/subscriptions/pending', async (req, res) => {
  try {
    ...
    console.error('Pending subscriptions error:', error);
    res.json({ success: false, pending: [], error: error.message });
  }
});

// POST /api/subscriptions/pending/:id/confirm - Confirm a pending subscription
app.post('/api/subscriptions/pending/:id/confirm', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    if (fetchError || !pending) {
      return res.json({ success: false, error: 'Pending subscription not found' });
    }

    const subscription = {
      vendor_name: updates.vendor_name || pending.vendor_name,
      amount: pending.detected_amount,
      currency: pending.detected_currency || 'AUD',
      billing_cycle: pending.detected_cycle || 'monthly',
      account_status: 'active',
      category: updates.category || 'operations',
      discovery_source: [pending.discovery_source],
      discovery_confidence: pending.discovery_confidence,
      expected_amount: pending.detected_amount,
      notes: `Discovered via ${pending.discovery_source} with ${pending.discovery_confidence}% confidence`,
      ...updates
    };
    ...
    await supabase
      ...
      .update({
        status: 'confirmed',
        resolved_subscription_id: newSub.id,
        resolved_at: new Date().toISOString(),
        resolved_by: 'api'
      })
      .eq('id', id);
    ...
    await supabase
      ...
      .insert({
        ...
        source: pending.discovery_source,
        confidence: pending.discovery_confidence,
        vendor_name: pending.vendor_name,
        details: { from_pending_id: id }
      });
...
// POST /api/subscriptions/pending/:id/reject - Reject a pending subscription
app.post('/api/subscriptions/pending/:id/reject', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    res.json({ success: true, message: 'Pending subscription rejected' });
  } catch (error) {
...
app.get('/api/subscriptions/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, subscription, history: history || [] });
  } catch (e) {
...
app.post('/api/subscriptions', async (req, res) => {
  try {
    const { name, provider, category, billing_cycle, cost_per_cycle, currency, renewal_date, project_codes, notes } = req.body;
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        cost_per_cycle: cost_per_cycle || 0,
...
app.patch('/api/subscriptions/:id', async (req, res) => {
  try {
    ...
    if (updates.cost_per_cycle !== undefined) {
      updates.amount = updates.cost_per_cycle;
      delete updates.cost_per_cycle;
    }
    if (updates.name !== undefined) {
      updates.vendor_name = updates.name;
      delete updates.name;
    }
    if (updates.provider !== undefined) {
      if (!updates.vendor_name) {
        updates.vendor_name = updates.provider;
      }
      delete updates.provider;
    }
    // Remove renewal_date if empty string (frontend sends '' for empty dates)
    if (updates.renewal_date === '') {
      delete updates.renewal_date;
    }

    // Get current for history
    const { data: current } = await supabase
      .from('subscriptions')
      .select('amount, account_status')
      .eq('id', id)
      .single();
    ...
    if (error) {
      console.error('Subscription update DB error:', error);
      throw error;
    }

    // Log history if cost or status changed
    if (updates.amount !== undefined || updates.account_status !== undefined) {
      await supabase.from('subscription_history').insert({
        subscription_id: id,
        change_type: updates.account_status === 'cancelled' ? 'cancelled' :
                     updates.amount !== current?.amount ? 'price_change' : 'renewed',
        previous_cost: current?.amount,
        new_cost: updates.amount,
        previous_status: current?.account_status,
    ...
    const response = {
      ...
      provider: data.vendor_name,
      cost_per_cycle: parseFloat(data.amount) || 0,
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// RECEIPT RECONCILIATION ENDPOINTS
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

// Mock receipts data (fallback)
function getMockReceiptsData() {
  return [
    { id: 'rx_1', vendor: 'Officeworks', amount: 234.50, currency: 'AUD', transaction_date: '2026-01-24', status: 'pending', category: 'office', suggested_match: 'Receipt from Dext', match_confidence: 92 },
    { id: 'rx_2', vendor: 'Unknown Merchant', amount: 89.00, currency: 'AUD', transaction_date: '2026-01-23', status: 'pending', category: 'other', suggested_match: null, match_confidence: 0 },
    { id: 'rx_3', vendor: 'AWS', amount: 1250.00, currency: 'USD', transaction_date: '2026-01-22', status: 'email_suggested', category: 'infrastructure', suggested_email_subject: 'Your AWS invoice for January 2026', match_confidence: 95 },
    { id: 'rx_4', vendor: 'Bunnings', amount: 156.80, currency: 'AUD', transaction_date: '2026-01-21', status: 'pending', category: 'operations', suggested_match: null, match_confidence: 0 },
    { id: 'rx_5', vendor: 'Uber Eats', amount: 45.90, currency: 'AUD', transaction_date: '2026-01-20', status: 'pending', category: 'meals', suggested_match: 'Email receipt', match_confidence: 88 },
  ];
}

// Get unmatched transactions
app.get('/api/receipts/unmatched', async (req, res) => {
  try {
    ...
    let query = supabase
      ...
      .gte('transaction_date', '2025-10-01')  // Only receipts from Oct 1, 2025 onwards
    ...
    if (error || finalReceipts.length === 0) {
      console.log('Using mock receipts data:', error?.message);
      finalReceipts = getMockReceiptsData();
      if (category) {
        finalReceipts = finalReceipts.filter(r => r.category === category);
      }
    }

    res.json({
      success: true,
      receipts: finalReceipts.slice(0, parseInt(limit)),
      count: finalReceipts.length
    });
  } catch (e) {
...
app.get('/api/receipts/score', async (req, res) => {
  try {
    // Get week's reconciliation stats
    ...
    // If all tables don't exist, return mock data
    if (weekError && pendingError && statsError) {
      console.log('Using mock receipt score data');
      return res.json({
        success: true,
        score: 82,
        pending: 5,
        ...
        totalPoints: 1250,
        achievements: [
          { name: '5-Week Streak', icon: '🏆', earned_at: '2026-01-20' },
          { name: 'Zero Inbox Hero', icon: '🎯', earned_at: '2026-01-15' },
          { name: 'Speed Demon', icon: '⚡', earned_at: '2026-01-10' }
        ]
      });
    }
    ...
    const score = total > 0 ? Math.round((resolvedThisWeek / total) * 100) : 100;
    ...
    res.json({
      ...
      streak: stats?.current_streak || weekData?.streak || 0,
...
app.get('/api/receipts/suggestions/:txId', async (req, res) => {
  try {
    ...
    // Return existing suggestion if present
    const suggestions = [];
...
// Mark receipt as matched/resolved
app.post('/api/receipts/match', async (req, res) => {
  try {
    const { receiptId, matchType = 'manual' } = req.body;
...
// Skip a receipt
app.post('/api/receipts/skip', async (req, res) => {
  try {
    const { receiptId, reason = 'no_receipt_needed' } = req.body;
...
// Get achievements
app.get('/api/receipts/achievements', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: e.message, achievements: [] });
  }
});

// Scan for missing receipts (run detection)
app.post('/api/receipts/scan', async (req, res) => {
  try {
    const { daysBack = 90, skipAI = false } = req.body;

    // Dynamic import of the receipt detector
    const { detectMissingReceipts, saveMissingReceipts, categorizeVendor } = await import('../../scripts/lib/receipt-detector.mjs');

    console.log(`Running receipt scan (last ${daysBack} days)...`);
    ...
    // Optionally run AI matching
    let matched = 0;
    if (!skipAI && saveResults.inserted > 0) {
      try {
        const { findEmailMatches, suggestEmailMatch } = await import('../../scripts/lib/receipt-matcher.mjs');
        const { getPendingReceipts } = await import('../../scripts/lib/receipt-detector.mjs');

        const pending = await getPendingReceipts({ statuses: ['pending'], limit: 20 });

        for (const receipt of pending) {
          const matches = await findEmailMatches(receipt, { minScore: 40 });
        ...
        console.error('AI matching error (non-fatal):', matchError.message);
      }
    }
    ...
    res.json({
      ...
      summary: {
        ...
        byCategory: results.summary.byCategory
      }
    });
  } catch (e) {
...
// Unified receipt search across all sources
app.post('/api/receipts/search', async (req, res) => {
  try {
    const { query, sources = ['xero', 'gmail', 'calendar'], dateRange = {}, amount = {} } = req.body;

    if (!query) {
      return res.status(400).json({ error: 'query is required' });
    }

    // Dynamic import of the unified search module
    const { unifiedReceiptSearch } = await import('../../scripts/lib/unified-receipt-search.mjs');

    console.log(`Unified search: "${query}" across ${sources.join(', ')}`);
...
// Get calendar events near a transaction date (travel context)
app.get('/api/receipts/calendar-context/:txDate', async (req, res) => {
  try {
    const { txDate } = req.params;
    const windowDays = parseInt(req.query.days) || 7;

    const { findNearbyTravelEvents } = await import('../../scripts/lib/receipt-matcher.mjs');

    const events = await findNearbyTravelEvents(txDate, windowDays);
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// PROJECT CODES ENDPOINT
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

app.get('/api/project-codes', async (req, res) => {
  try {
    const projectCodesPath = join(__dirname, '../../config/project-codes.json');
    const content = await readFile(projectCodesPath, 'utf-8');
    const projectCodes = JSON.parse(content);

    const codes = Object.entries(projectCodes.projects || {}).map(([code, data]) => ({
      code,
      name: data.name,
    ...
    res.json({
      success: true,
      codes,
      categories: projectCodes.categories || {}
    });
  } catch (e) {
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// XERO SPENDING BY PROJECT
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

app.get('/api/xero/spending-by-project', async (req, res) => {
  try {
    const { months = 3 } = req.query;
    const startDate = new Date();
    startDate.setMonth(startDate.getMonth() - parseInt(months));

    // Get expenses (ACCPAY invoices) with tracking categories
    const { data: invoices, error } = await supabase
      .from('xero_invoices')
      .select('total, tracking_categories, status, date')
      .eq('type', 'ACCPAY')
    ...
    // Group by tracking category (project)
    const byProject = {};
    (invoices || []).forEach(inv => {
      // tracking_categories is usually an array like [{name: 'Project', option: 'JusticeHub'}]
      const tracking = inv.tracking_categories || [];
      let projectName = 'Unassigned';

      for (const t of tracking) {
        if (t.name === 'Project' || t.name === 'project') {
          projectName = t.option || 'Unassigned';
          break;
        }
      }

      if (!byProject[projectName]) {
        byProject[projectName] = { name: projectName, total: 0, count: 0 };
      }
      byProject[projectName].total += Math.abs(inv.total || 0);
...
app.post('/api/v3/agent/query', async (req, res) => {
  try {
    ...
    // Get goals context
    try {
      const goalsRes = await fetch('http://localhost:3456/api/goals/2026');
      if (goalsRes.ok) {
        const goalsData = await goalsRes.json();
        context.goals = goalsData;
      }
    } catch (e) { /* goals unavailable */ }

    // Get projects context
    try {
      const projectsRes = await fetch('http://localhost:3456/api/projects');
      if (projectsRes.ok) {
        const projectsData = await projectsRes.json();
        context.projects = projectsData;
      }
    } catch (e) { /* projects unavailable */ }

    // Get financial context
    try {
      const financialRes = await fetch('http://localhost:3456/api/financial/summary');
      if (financialRes.ok) {
        const financialData = await financialRes.json();
        context.financial = financialData;
      }
    } catch (e) { /* financial unavailable */ }
    ...
    if (queryLower.includes('cash flow') || queryLower.includes('money') || queryLower.includes('financial')) {
      const financial = context.financial;
      if (financial?.summary) {
        response = `Your current financial position:\n\n` +
          `• Total transactions: ${financial.summary.totalTransactions.toLocaleString()}\n` +
          `• Net position: ${financial.summary.totalMoneyIn > financial.summary.totalMoneyOut ? 'Positive' : 'Needs attention'}\n` +
          `• Unmatched transactions: ${financial.summary.unmatchedTransactions || 0}\n\n` +
          `Recommendation: Review unmatched transactions to improve financial clarity.`;
      } else {
        response = 'Financial data is currently being synchronized. Check back shortly for detailed cash flow insights.';
      }
    } else if (queryLower.includes('grant') || queryLower.includes('funding') || queryLower.includes('opportunity')) {
      response = `🎯 Grant & Funding Opportunities for ACT:\n\n` +
        `• Regional Arts Fund - Deadline approaching\n` +
        `• R&D Tax Incentive - Up to $46K potential\n` +
        `• Google AI Grants - Rolling applications\n\n` +
        `Contact me for detailed application assistance.`;
    } else if (queryLower.includes('project') || queryLower.includes('what should')) {
      const projectCount = context.projects?.length || 0;
      response = `You have ${projectCount} active projects in your portfolio.\n\n` +
        `Focus areas include:\n` +
        `• Client delivery (JusticeHub, Seeva, etc.)\n` +
        `• Platform development (ACT Intelligence Platform)\n` +
        `• Research initiatives\n\n` +
        `Would you like me to analyze specific project profitability?`;
    } else if (queryLower.includes('compliance') || queryLower.includes('bas') || queryLower.includes('tax')) {
      response = `🇦🇺 Australian Compliance Status:\n\n` +
        `• BAS: Q3 2025 due Oct 28 (~ $61K GST)\n` +
        `• Tax: R&D claim in progress\n` +
        `• Super: Up to date\n\n` +
        `Recommendation: Lodge BAS by deadline to avoid ATO penalties.`;
    } else {
      response = `I understand you're asking about: "${query}"\n\n` +
        `I have access to your business data including:\n` +
        `• Financial transactions and cash flow\n` +
        `• Projects and client work\n` +
        `• Compliance deadlines (BAS, GST, Tax)\n` +
        `• Grant opportunities\n\n` +
        `Could you provide more details about what you'd like to know? I'm here to help with business intelligence, financial insights, and compliance matters.`;
    }
...
// Real calendar data from Supabase
async function getRealCalendarEvents(options = {}) {
  try {
    const { start, end, type, limit } = options;
    const now = new Date();

    // Build query
    let query = supabase
      .from('calendar_events')
      .select('id, google_event_id, title, description, start_time, end_time, location, attendees, event_type, status')
      .order('start_time', { ascending: true });
    ...
    const todayEvents = formattedEvents.filter(e =>
      new Date(e.start) >= todayStart && new Date(e.start) < todayEnd
    );
    const weekEvents = formattedEvents.filter(e =>
      new Date(e.start) >= todayStart && new Date(e.start) < weekEnd
    );
    const monthEvents = formattedEvents.filter(e =>
      new Date(e.start) >= todayStart && new Date(e.start) <= monthEnd
    );
    ...
    return {
      ...
      summary: {
        ...
        this_week: weekEvents.length,
        this_month: monthEvents.length,
        meetings: formattedEvents.filter(e => e.type === 'meeting').length,
        deadlines: formattedEvents.filter(e => e.type === 'deadline').length,
        tasks: formattedEvents.filter(e => e.type === 'task').length
      },
      source: 'supabase',
    ...
    return {
      events: [],
      summary: { today: 0, this_week: 0, this_month: 0, meetings: 0, deadlines: 0, tasks: 0 },
      source: 'error',
      error: error.message
    };
  }
...
app.get('/api/calendar/events', async (req, res) => {
  try {
    ...
    const data = await getRealCalendarEvents({ start, end, type });
    res.json({
      success: true,
      ...data
    });
...
app.get('/api/calendar/summary', async (req, res) => {
  try {
    // Get events for the current month for summary
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();
    const monthEnd = new Date(now.getFullYear(), now.getMonth() + 1, 0).toISOString();

    const data = await getRealCalendarEvents({ start: monthStart, end: monthEnd });
    res.json({
      success: true,
      ...data.summary
    });
...
app.get('/api/calendar/upcoming', async (req, res) => {
  try {
    ...
    const now = new Date().toISOString();
    const data = await getRealCalendarEvents({ start: now, limit });
    res.json({
      success: true,
      events: data.events
    });
...
// Get emails from communications_history (synced email data)
app.get('/api/emails/recent', async (req, res) => {
  try {
    ...
    const { data: emails, error } = await supabase
      .from('communications_history')
      .select('id, subject, content_preview, occurred_at, direction, is_reply, requires_response, metadata')
      .eq('channel', 'email')
      .order('occurred_at', { ascending: false })
      .limit(parseInt(limit));
    ...
    const transformed = (emails || []).map(e => ({
      ...
      snippet: e.content_preview?.substring(0, 100) || '',
      date: e.occurred_at,
      direction: e.direction,
      isReply: e.is_reply,
      requiresResponse: e.requires_response,
      read: !e.metadata?.labels?.includes('UNREAD')
    }));

    res.json({ success: true, emails: transformed });
  } catch (e) {
    console.error('Emails recent error:', e);
    res.status(500).json({ error: e.message, emails: [] });
  }
});

// Get email stats from communications_history
app.get('/api/emails/stats', async (req, res) => {
  try {
    // Count unread (has UNREAD in labels)
    ...
    const unread = (emails || []).filter(e => e.metadata?.labels?.includes('UNREAD')).length;
    const requiresResponse = (emails || []).filter(e => e.requires_response).length;
    ...
    res.status(500).json({ error: e.message, unread: 0, requiresResponse: 0, todayCount: 0 });
  }
});
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// BULK ENRICHMENT ENDPOINTS (Legacy v3 API Compatibility)
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

let bulkEnrichmentStatus = {
  total: 20398,
  enriched: 18420,
  remaining: 1978,
  progress: 90.3,
...
app.get('/api/v3/bulk/enrich/status', async (req, res) => {
  res.json({ success: true, status: bulkEnrichmentStatus });
});
...
app.get('/api/v3/bulk/enrich/preview', async (req, res) => {
  res.json({
    ...
    preview: {
      totalToEnrich: 1978,
      sampleContacts: [
        { name: 'John Smith', company: 'Tech Corp', position: 'CEO' },
        { name: 'Jane Doe', company: 'Innovation Ltd', position: 'CTO' },
        { name: 'Bob Wilson', company: 'Startup Inc', position: 'Founder' }
      ],
      estimatedTime: 45,
      estimatedCost: 197.80
    }
  });
});

app.post('/api/v3/bulk/enrich/start', async (req, res) => {
  bulkEnrichmentStatus.isRunning = true;
  bulkEnrichmentStatus.currentBatch = 1;
  res.json({ success: true, message: 'Bulk enrichment started' });
});

app.post('/api/v3/bulk/enrich/stop', async (req, res) => {
  bulkEnrichmentStatus.isRunning = false;
  bulkEnrichmentStatus.currentBatch = 0;
  res.json({ success: true, message: 'Bulk enrichment stopped' });
});

// =============================================================================
// V1 COMPATIBILITY ROUTES (for act-intelligence-platform frontend)
// =============================================================================

// Goals 2026 - maps to /api/goals/2026
app.get('/api/v1/goals/2026', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/goals/2026`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
app.get('/api/v1/goals/2026/calendar', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/goals/2026/calendar`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.status(500).json({ error: error.message });
...
// Ecosystem - maps to /api/ecosystem
app.get('/api/v1/ecosystem/projects', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/ecosystem`);
    const data = await response.json();
    res.json({ projects: data.sites || [] });
  } catch (error) {
    res.status(500).json({ projects: [], error: error.message });
  }
});

// Wiki - stub endpoints (knowledge base not yet implemented in real backend)
app.get('/api/v1/wiki/articles', (req, res) => {
  res.json({
    articles: [
      { id: '1', slug: 'philosophy', title: 'ACT Philosophy', category: 'philosophy', excerpt: 'Our approach to community regeneration', view_count: 42, updated_at: new Date().toISOString(), tags: ['core', 'values'] },
      { id: '2', slug: 'lcaa-framework', title: 'LCAA Framework', category: 'process', excerpt: 'Listen, Curiosity, Action, Art methodology', view_count: 38, updated_at: new Date().toISOString(), tags: ['process', 'framework'] },
      { id: '3', slug: 'handover', title: 'Handover Protocol', category: 'process', excerpt: 'How to transfer project ownership', view_count: 15, updated_at: new Date().toISOString(), tags: ['process', 'projects'] }
    ]
  });
});

app.get('/api/v1/wiki/categories', (req, res) => {
  res.json({
    categories: [
      { name: 'philosophy', count: 1, description: 'Core values and approach' },
      { name: 'process', count: 2, description: 'How we do things' },
      { name: 'project', count: 0, description: 'Project documentation' },
      { name: 'reference', count: 0, description: 'Reference materials' }
    ]
  });
});

app.get('/api/v1/wiki/search', (req, res) => {
  const { q } = req.query;
  res.json({ query: q, articles: [] });
});

// Studio - maps to /api/projects
app.get('/api/v1/studio/projects', async (req, res) => {
  try {
    ...
    const projects = (data.projects || []).map(p => ({
      ...
      end_date: p.deadline || '2026-12-31',
      contacts: p.team_members || [],
      intelligence: { risk_level: 'low', health_score: p.progress || 75 }
    }));
    res.json({ projects });
  } catch (error) {
    res.json({ projects: [], error: error.message });
  }
});

app.get('/api/v1/studio/stats', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/projects`);
    const data = await response.json();
    ...
    res.json({ total: 0, active: 0, completed: 0, total_budget: 0, by_category: [] });
  }
});

// Command Center - maps to /api/goals/*
app.get('/api/v1/command-center/morning-brief', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/goals/2026`);
    const data = await response.json();
    res.json({ brief: data, generated_at: new Date().toISOString() });
  } catch (error) {
    res.json({ brief: null, error: error.message });
  }
});

app.get('/api/v1/command-center/tasks', async (req, res) => {
  try {
...
app.get('/api/v1/command-center/overview', async (req, res) => {
  try {
    const [goalsRes, projectsRes] = await Promise.all([
      fetch(`http://localhost:${PORT}/api/goals/summary`),
      fetch(`http://localhost:${PORT}/api/projects`)
    ]);
    const goals = await goalsRes.json();
    const projects = await projectsRes.json();
    res.json({ goals, projects, timestamp: new Date().toISOString() });
  } catch (error) {
    res.json({ error: error.message });
  }
});

app.get('/api/v1/command-center/relationships', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/relationships/list`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.json({ relationships: [], error: error.message });
  }
});

// Integrations - maps to /api/*
app.get('/api/v1/integrations/status', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/integrations/status`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.json({ integrations: [], error: error.message });
  }
});

app.get('/api/v1/integrations/gmail/workspace/stats', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/gmail/recent`);
    const data = await response.json();
    res.json({ recent_count: (data.messages || []).length, unread: 0 });
  } catch (error) {
    res.json({ recent_count: 0, unread: 0, error: error.message });
  }
});

app.get('/api/v1/integrations/ghl/contacts', async (req, res) => {
  // Stub - GHL contacts would need GHL API integration
  res.json({ contacts: [], total: 0 });
});

// Search - maps to /api/search
app.get('/api/v1/search/unified', async (req, res) => {
  try {
    const { q } = req.query;
    const response = await fetch(`http://localhost:${PORT}/api/search?q=${q}`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.json({ query: '', results: {}, error: error.message });
  }
});

// Financial - maps to /api/financial
app.get('/api/v1/financial/summary', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/financial/summary`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.json({ summary: { total_income: 0, total_expenses: 0, balance: 0 }, error: error.message });
  }
});

// Projects - maps to /api/projects
...
app.get('/api/v1/projects/summary', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/projects`);
    const data = await response.json();
    const projects = data.projects || [];
    ...
    res.json({ summary: { total: 0, active: 0, completed: 0, total_budget: 0 }, error: error.message });
  }
});

// Contacts - maps to /api/contacts/* (would need actual contacts API)
app.get('/api/v1/contacts/all', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('ghl_contacts')
      .select('id, first_name, last_name, email, phone, tags, source')
      .limit(1000);
    if (error) throw error;
    res.json({ contacts: data || [], total: data?.length || 0 });
  } catch (error) {
    res.json({ contacts: [], total: 0, error: error.message });
  }
});

app.get('/api/v1/contacts/all/stats', async (req, res) => {
  try {
    const { data } = await supabase.from('ghl_contacts').select('source', { count: 'exact', head: true });
    res.json({ total: data || 0, by_source: {} });
  } catch (error) {
    res.json({ total: 0, by_source: {} });
  }
});

// Ecosystem health summary (for ACTBrainCenter)
app.get('/api/ecosystem/health-summary', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/ecosystem`);
    const data = await response.json();
    ...
    const summary = {
      ...
      offline: sites.filter(s => s.status === 'offline' || s.status === 'critical').length,
      avgScore: sites.length > 0 ? Math.round(sites.reduce((sum, s) => sum + (s.health_score || 0), 0) / sites.length) : 0
    };

    res.json({
      summary,
      sites,
      alerts: sites.filter(s => s.status === 'critical' || s.status === 'degraded').map(s => ({
        id: s.id,
        severity: s.status === 'critical' ? 'critical' : 'warning',
        message: `${s.name} is ${s.status} (health: ${s.health_score})`,
        created_at: s.last_check_at
      }))
    });
  } catch (error) {
    res.json({ summary: { total: 0, healthy: 0, degraded: 0, offline: 0, avgScore: 0 }, sites: [], alerts: [] });
  }
});

// Subscriptions - handle missing tenantId with default
app.get('/api/v1/subscriptions', async (req, res) => {
  try {
    // Subscriptions table may not exist - return stub data
    // In production, this would query email_financial_documents for subscription patterns
    res.json({
      success: true,
      data: [
        { id: '1', name: 'Claude Pro', provider: 'Anthropic', amount: 20, currency: 'USD', interval: 'month', confidence: 0.95 },
        { id: '2', name: 'GitHub Copilot', provider: 'GitHub', amount: 10, currency: 'USD', interval: 'month', confidence: 0.90 },
        { id: '3', name: 'Supabase', provider: 'Supabase', amount: 25, currency: 'USD', interval: 'month', confidence: 0.85 }
      ],
      count: 3,
      monthlyTotal: { USD: 55 },
      yearlyProjection: { USD: 660 }
    });
  } catch (error) {
    res.json({ success: false, data: [], count: 0, error: error.message });
  }
});
...
app.get('/api/intelligence/dashboard', async (req, res) => {
  try {
    ...
    const { data: relationships } = await supabase.from('relationship_health').select('overall_score').limit(100);
    const avgRelationshipScore = relationships?.length > 0
      ? Math.round(relationships.reduce((sum, r) => sum + (r.overall_score || 0), 0) / relationships.length)
      : 0;

    // Get knowledge stats
    const { count: knowledgeCount } = await supabase.from('knowledge_chunks').select('*', { count: 'exact', head: true });

    res.json({
      dashboard: {
        relationshipHealth: { score: avgRelationshipScore, trend: 'stable' },
        knowledgeBase: { chunks: knowledgeCount || 0, lastUpdated: new Date().toISOString() },
        communicationVolume: { today: 0, week: 0, month: 0 },
        entityCount: 14079
      }
    });
  } catch (error) {
    res.json({ dashboard: null, error: error.message });
  }
});

app.get('/api/intelligence/patterns', async (req, res) => {
  try {
    // Get patterns from canonical_entities
    const { data: entities } = await supabase
      .from('canonical_entities')
    ...
    res.json({ patterns: null, error: error.message });
  }
});
...
app.get('/api/intelligence/relationships/graph', async (req, res) => {
  try {
    const { data: relationships } = await supabase
      .from('relationship_health')
      .select('contact_id, overall_score, last_interaction, next_action')
      .limit(100);

    const nodes = [
      { id: 'me', label: 'ACT', type: 'self' },
      ...(relationships?.map((r, i) => ({
        id: r.contact_id || `contact_${i}`,
        label: `Contact ${i + 1}`,
        type: 'contact',
    ...
    const links = relationships?.map(r => ({
      source: 'me',
      target: r.contact_id || 'unknown',
      strength: (r.overall_score || 50) / 100
    })) || [];

    res.json({ nodes, links });
  } catch (error) {
    res.json({ nodes: [], links: [], error: error.message });
  }
});

app.post('/api/intelligence/scan', async (req, res) => {
  // Trigger intelligence scan - returns queued job
  res.json({
    scan: {
      id: `scan_${Date.now()}`,
      status: 'queued',
...
// Bookkeeping progress endpoint
app.get('/api/bookkeeping/progress', async (req, res) => {
  try {
    // Get financial documents stats
    const { count: total } = await supabase.from('email_financial_documents').select('*', { count: 'exact', head: true });
    const { count: processed } = await supabase.from('email_financial_documents').select('*', { count: 'exact', head: true }).eq('is_processed', true);
    const { count: subscriptions } = await supabase.from('email_financial_documents').select('*', { count: 'exact', head: true }).eq('is_subscription', true);

    res.json({
      checklistProgress: total > 0 ? Math.round((processed / total) * 100) : 0,
      overdueInvoices: 0, // Would need Xero integration
      pendingReceipts: total - processed,
      nextBASDue: null, // Would need Xero integration
      gstOwed: 0, // Would need Xero integration
      subscriptionsCount: subscriptions || 0,
      totalDocuments: total || 0
    });
  } catch (error) {
    res.json({ checklistProgress: 0, overdueInvoices: 0, pendingReceipts: 0, nextBASDue: null, gstOwed: 0 });
  }
});

app.post('/api/bookkeeping/chase-all', async (req, res) => {
  // Chase all overdue invoices - stub for now
  res.json({
    success: true,
    message: 'Chase emails queued',
    emails_sent: 0,
    emails_queued: 0
  });
});
...
app.get('/api/v1/projects/summary', async (req, res) => {
  try {
    ...
    const summary = {
      ...
      active: projects.filter(p => p.status === 'active').length,
      completed: projects.filter(p => p.status === 'completed').length,
      atRisk: projects.filter(p => p.status === 'at_risk' || p.status === 'in_progress').length,
      byPhase: {
        listen: projects.filter(p => p.category === 'listen' || p.lane === 'A').length,
        curiosity: projects.filter(p => p.category === 'curiosity' || p.lane === 'B').length,
        action: projects.filter(p => p.category === 'action' || p.lane === 'C').length,
        art: projects.filter(p => p.category === 'art' || p.lane === 'D').length
      },
    ...
    res.json({ summary: { total: 0, active: 0, completed: 0, atRisk: 0, byPhase: {}, totalBudget: 0 } });
  }
});
...
// Wiki categories endpoint
app.get('/api/v1/wiki/categories', async (req, res) => {
  try {
    // Query Supabase for wiki articles grouped by category
    ...
    const categoryCount = {};
    articles?.forEach(a => {
      const cat = a.metadata?.category || 'uncategorized';
    ...
    res.json({ categories: [
      { name: 'philosophy', count: 1, description: 'Core values' },
      { name: 'process', count: 2, description: 'How we do things' },
      { name: 'project', count: 0, description: 'Project documentation' },
      { name: 'reference', count: 0, description: 'Reference materials' }
    ]});
...
// Search unified (alias for /api/search)
app.get('/api/v1/search/unified', async (req, res) => {
  try {
    const { q } = req.query;
    const response = await fetch(`http://localhost:${PORT}/api/search?q=${q || ''}`);
    ...
    res.json({ query: q, results: {}, error: error.message });
  }
});

// Goals summary - wrap in summary object for frontend compatibility
app.get('/api/v1/goals/summary', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/goals/2026`);
    const data = await response.json();
    ...
    const summary = {
      ...
      in_progress: goals.filter(g => g.status === 'In progress' || (g.progress_percentage > 0 && g.progress_percentage < 100)).length,
      not_started: goals.filter(g => !g.progress_percentage || g.progress_percentage === 0).length,
      overallProgress: goals.length > 0
        ? Math.round(goals.reduce((sum, g) => sum + (g.progress_percentage || 0), 0) / goals.length)
        : 0,
      byLane: Object.entries(lanes).map(([name, laneData]) => ({
        lane: name,
        count: laneData.goals?.length || 0,
        completed: laneData.completed || 0
      }))
    };

    // Wrap in summary object for frontend compatibility
    res.json({ summary });
  } catch (error) {
    res.json({ summary: { total: 0, completed: 0, in_progress: 0, not_started: 0, overallProgress: 0, byLane: [] } });
  }
});

// Bookkeeping progress - wrap for v1 compatibility
app.get('/api/v1/bookkeeping/progress', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/bookkeeping/progress`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.json({ success: false, summary: { total_income: 0, total_expenses: 0, balance: 0 }, error: error.message });
  }
});

// Agents - wrap in agents object for frontend compatibility
app.get('/api/v1/agents', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/agents`);
    const data = await response.json();
    // API returns array directly, wrap in object
    res.json({ agents: Array.isArray(data) ? data : data.agents || [] });
  } catch (error) {
    res.json({ agents: [], error: error.message });
  }
});

// Communications - wrap in communications object for frontend compatibility
app.get('/api/v1/communications/recent', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/communications/recent`);
    const data = await response.json();
    // Transform to frontend format
    const communications = data.recent || data.communications || (Array.isArray(data) ? data : []);
    const total = data.today || data.total || communications.length;
    res.json({ communications, total });
  } catch (error) {
    res.json({ communications: [], total: 0, error: error.message });
  }
});

// Ecosystem stats - ensure it returns proper stats format
app.get('/api/v1/ecosystem/stats', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/ecosystem`);
    const data = await response.json();
    const sites = data.sites || [];
    ...
    res.json({
      total: sites.length,
      healthy: sites.filter(s => s.status === 'healthy').length,
      needs_attention: sites.filter(s => s.status === 'degraded' || s.status === 'critical').length,
      avg_health: sites.length > 0 ? Math.round(sites.reduce((sum, s) => sum + (s.health_score || 0), 0) / sites.length) : 0,
      by_category: Object.entries(
        sites.reduce((acc, s) => {
    ...
    res.json({ total: 0, healthy: 0, needs_attention: 0, avg_health: 0, by_category: [] });
  }
});

// Ecosystem health summary - for EcosystemCompendium component
app.get('/api/v1/ecosystem/health', async (req, res) => {
  try {
    const response = await fetch(`http://localhost:${PORT}/api/ecosystem/health-summary`);
    const data = await response.json();
    res.json(data);
  } catch (error) {
    res.json({ summary: { total: 0, healthy: 0, degraded: 0, offline: 0 }, sites: [], alerts: [] });
  }
});

// Intelligence geo-alerts - for ProjectDetail component
app.get('/api/intelligence/geo-alerts', async (req, res) => {
  try {
    // Return empty geo alerts for now - geo intelligence not yet implemented
...
// Stories endpoint - for StoryManagement component
app.get('/api/stories', async (req, res) => {
  try {
    // Return empty stories for now - story management not yet implemented
    res.json({ stories: [], total: 0 });
  } catch (error) {
    res.json({ stories: [], total: 0, error: error.message });
  }
});

// V3 Agent endpoints - for ACTBusinessAgent component
...
app.get('/api/v3/agent/projects/health', async (req, res) => {
  try {
    res.json({
      projects: [],
      summary: { healthy: 0, atRisk: 0, offline: 0 }
    });
  } catch (error) {
    res.json({ error: error.message });
  }
...
app.post('/api/v3/agent/query', async (req, res) => {
  try {
    ...
    res.json({
      response: 'Agent query processing not yet implemented',
      query
    });
  } catch (error) {
    res.json({ error: error.message });
...
// Opportunities endpoint - for Opportunities sidebar item
app.get('/api/opportunities', async (req, res) => {
  try {
    res.json({
      opportunities: [],
      total: 0,
      categories: { revenue: [], partnership: [], growth: [] }
    });
  } catch (error) {
    res.json({ opportunities: [], total: 0, error: error.message });
  }
});

// Content endpoint - for Content sidebar item
app.get('/api/content', async (req, res) => {
  try {
    ...
    res.json({ articles: [], total: 0, error: error.message });
  }
});

// Goods endpoint - for Goods Wiki sidebar item
app.get('/api/goods', async (req, res) => {
  try {
    ...
    res.json({ goods: [], total: 0, error: error.message });
  }
});
...
app.get('/api/v2/direction/scorecard', async (req, res) => {
  try {
    // Aggregate key metrics from various tables
    const [
      { count: contactsCount },
      ...
      supabase.from('ghl_contacts').select('*', { count: 'exact', head: true }),
      supabase.from('agent_proposals').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
      supabase.from('agent_task_queue').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
      supabase.from('act_projects').select('*', { count: 'exact', head: true }),
      supabase.from('communications_history').select('*', { count: 'exact', head: true }),
      supabase.from('xero_invoices').select('*', { count: 'exact', head: true }).gt('amount_due', 0)
    ]);

    // Get relationship health
    const { data: healthData } = await supabase.rpc('get_relationship_health_summary').single();
    ...
    res.json({
      ...
      scorecard: {
        relationships: {
          ...
          hot: healthData?.hot || 0,
          warm: healthData?.warm || 0,
          cool: healthData?.cool || 0
        },
        agents: {
        ...
        operations: {
          ...
          outstandingInvoices: invoicesCount || 0
        }
      },
      meta: {
        timestamp: new Date().toISOString(),
    ...
    res.json({
      success: false,
      scorecard: {
        relationships: { total: 0, hot: 0, warm: 0, cool: 0 },
        agents: { pendingProposals: 0, pendingTasks: 0 },
        operations: { projects: 0, communications: 0, outstandingInvoices: 0 }
      },
      error: error.message
    });
  }
...
app.get('/api/v2/monitoring/integrations', async (req, res) => {
  try {
    // Check sync state for each integration
    const { data: syncStates } = await supabase
      .from('sync_state')
      .select('*')
    ...
    const { data: xeroSyncLogs } = await supabase
      .from('xero_sync_log')
      .select('*')
    ...
    const integrations = [
      {
        ...
        service: 'ghl',
        status: 'connected',
        lastSync: syncStates?.find(s => s.service === 'ghl')?.last_sync_at || null,
        errorCount: syncStates?.find(s => s.service === 'ghl')?.error_count || 0
      },
      {
        name: 'Google Calendar',
        service: 'calendar',
        status: 'connected',
        lastSync: syncStates?.find(s => s.service === 'calendar')?.last_sync_at || null,
        errorCount: syncStates?.find(s => s.service === 'calendar')?.error_count || 0
      },
      {
        name: 'Gmail',
        service: 'gmail',
        status: 'connected',
        lastSync: syncStates?.find(s => s.service === 'gmail')?.last_sync_at || null,
        errorCount: syncStates?.find(s => s.service === 'gmail')?.error_count || 0
      },
      {
        name: 'Xero',
        service: 'xero',
        status: xeroSyncLogs?.[0]?.status === 'completed' ? 'connected' : 'warning',
        lastSync: xeroSyncLogs?.[0]?.completed_at || null,
        errorCount: xeroSyncLogs?.[0]?.errors?.length || 0
      },
      {
        name: 'Notion',
        service: 'notion',
        status: 'connected',
        lastSync: syncStates?.find(s => s.service === 'notion')?.last_sync_at || null,
        errorCount: syncStates?.find(s => s.service === 'notion')?.error_count || 0
      }
    ];

    // Calculate overall health
    const connectedCount = integrations.filter(i => i.status === 'connected').length;
    const overallHealth = connectedCount === integrations.length ? 'healthy' :
                         connectedCount >= integrations.length / 2 ? 'degraded' : 'critical';
...
app.get('/api/v1/subscriptions', async (req, res) => {
  try {
    ...
    // Get cost summary
    const { data: costs } = await supabase.from('v_subscription_costs').select('*');

    const totalMonthly = costs?.reduce((sum, c) => sum + Number(c.monthly_cost || 0), 0) || 0;
    const totalAnnual = costs?.reduce((sum, c) => sum + Number(c.annual_cost || 0), 0) || 0;
    ...
    res.json({ success: false, subscriptions: [], error: error.message });
  }
});

app.get('/api/v1/subscriptions/renewals', async (req, res) => {
  try {
    ...
    const { data: renewals } = await supabase
      ...
      .select('*')
      .lte('days_until_renewal', parseInt(days));
    ...
    res.json({ success: false, renewals: [], error: error.message });
  }
});
...
// Import discovery functions (lazy load to avoid circular deps)
let discoveryModule = null;
async function getDiscoveryModule() {
  if (!discoveryModule) {
    discoveryModule = await import('../../scripts/lib/subscription-discovery.mjs');
  }
  return discoveryModule;
}

// POST /api/subscriptions/discover - Run discovery scan
app.post('/api/subscriptions/discover', async (req, res) => {
  try {
    ...
    const discovery = await getDiscoveryModule();
    ...
    res.json({
      ...
      new_subscriptions: results.new_subscriptions,
      price_changes: results.price_changes,
      possibly_cancelled: results.possibly_cancelled,
...
// GET /api/subscriptions/discover - Get discovery status and alerts (no scan)
app.get('/api/subscriptions/discover', async (req, res) => {
  try {
    const discovery = await getDiscoveryModule();
    const status = await discovery.getDiscoveryStatus();

    if (status.error) {
      return res.json({ success: false, error: status.error });
    }

    res.json({
      success: true,
      pending_count: status.pending_count,
      pending_subscriptions: status.pending_subscriptions,
      alerts_count: status.alerts_count,
      subscription_alerts: status.subscription_alerts,
...
app.post('/api/subscriptions/pending/:id/confirm', async (req, res) => {
  try {
    const { id } = req.params;
    const updates = req.body; // Optional overrides: name, category, etc.
    ...
    // Create new subscription from pending
    const subscription = {
      name: updates.name || pending.vendor_name,
      provider: pending.vendor_name,
      category: updates.category || 'operations',
      billing_cycle: pending.detected_cycle || 'monthly',
      cost_per_cycle: pending.detected_amount,
      currency: pending.detected_currency || 'AUD',
      status: 'active',
      discovery_source: [pending.discovery_source],
      discovery_confidence: pending.discovery_confidence,
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// BUNYA - PROJECT HEALTH SCOUT
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

app.get('/api/scouts/bunya', async (req, res) => {
  try {
    // Get all project health records
    const { data: projects, error } = await supabase
      .from('project_health')
      .select('*')
    ...
    const avgScore = totalProjects > 0
      ? Math.round(projects.reduce((sum, p) => sum + (p.overall_score || 0), 0) / totalProjects)
      : 0;
    ...
    res.json({
      ...
      summary: {
        ...
        byStatus: summary || [],
        projectsNeedingAttention: needsAttention?.length || 0
      },
    ...
    console.error('BUNYA error:', error);
    res.json({
      success: false,
      summary: { totalProjects: 0, averageScore: 0, byStatus: [], projectsNeedingAttention: 0 },
      needsAttention: [],
      projects: [],
      error: error.message
    });
...
app.get('/api/scouts/bunya/:projectCode', async (req, res) => {
  try {
    const { projectCode } = req.params;
    ...
    console.error('BUNYA project error:', error);
    res.json({ success: false, health: null, error: error.message });
  }
});
...
app.get('/api/scouts/alta', async (req, res) => {
  try {
    ...
    const { data: opportunities, error } = await supabase
      .from('v_open_grant_opportunities')
      .select('*');
    ...
    const { data: pipeline } = await supabase
      .from('v_grant_pipeline')
      .select('*');

    // Get metrics
    const { data: metrics } = await supabase
      .from('v_grant_metrics')
      .select('*')
      .single();

    // High-fit opportunities (fit_score >= 70)
    const highFit = opportunities?.filter(o => (o.fit_score || 0) >= 70) || [];

    // Urgent deadlines (closing in 14 days)
    const urgent = opportunities?.filter(o => o.days_until_close !== null && o.days_until_close <= 14) || [];

    res.json({
      success: true,
      summary: {
        openOpportunities: opportunities?.length || 0,
        highFitOpportunities: highFit.length,
        urgentDeadlines: urgent.length,
        activeApplications: metrics?.active_count || 0,
        successRate: metrics?.success_rate || 0,
        totalAwarded: metrics?.total_awarded || 0
      },
    ...
    res.json({
      success: false,
      summary: { openOpportunities: 0, highFitOpportunities: 0, urgentDeadlines: 0, activeApplications: 0 },
      opportunities: [],
      pipeline: [],
      error: error.message
    });
...
app.get('/api/scouts/alta/opportunities/:id', async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Get any applications for this opportunity
    ...
    console.error('ALTA opportunity error:', error);
    res.json({ success: false, opportunity: null, error: error.message });
  }
});
...
app.get('/api/v2/events/stream', (req, res) => {
  // Set headers for SSE
...
// Endpoint to trigger events (for testing or internal use)
app.post('/api/v2/events/emit', (req, res) => {
  const { type, data } = req.body;
  if (!type) {
    return res.status(400).json({ error: 'Event type required' });
  }
  broadcastEvent(type, data || {});
  res.json({ success: true, clientCount: sseClients.size });
});

// Hook into database changes via Supabase realtime (if configured)
// This would require SUPABASE_REALTIME to be enabled
  ...
  try {
    ...
    const proposalsChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_proposals' }, (payload) => {
        broadcastEvent('proposal_change', { action: payload.eventType, record: payload.new || payload.old });
      })
      .subscribe();

    // Subscribe to task queue changes
    const tasksChannel = supabase
      .channel('agent_task_queue_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'agent_task_queue' }, (payload) => {
        broadcastEvent('task_change', { action: payload.eventType, record: payload.new || payload.old });
      })
      .subscribe();

    // Subscribe to relationship health changes
    const healthChannel = supabase
      .channel('relationship_health_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'relationship_health' }, (payload) => {
        broadcastEvent('relationship_change', { action: payload.eventType, record: payload.new || payload.old });
      })
      .subscribe();

    console.log('Supabase realtime subscriptions set up');
  } catch (error) {
    console.log('Supabase realtime not available:', error.message);
  }
}
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
// WIKI / COMPENDIUM ENDPOINTS
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

const COMPENDIUM_PATH = join(homedir(), 'Code/act-regenerative-studio/compendium');

// Get wiki structure (sections and pages)
app.get('/api/wiki/structure', async (req, res) => {
  try {
    const sections = [];
    const entries = await readdir(COMPENDIUM_PATH, { withFileTypes: true });

    for (const entry of entries) {
      if (entry.isDirectory() && !entry.name.startsWith('.') && entry.name !== 'scripts' && entry.name !== 'node_modules') {
        const sectionPath = join(COMPENDIUM_PATH, entry.name);
        const pages = [];
        ...
        async function scanDir(dirPath, prefix = '') {
          ...
          for (const item of items) {
            ...
            if (item.isDirectory()) {
              ...
              // Fall back to first H1 (but skip section headers like "Consent & Cultural Review")
              if (!title) {
                const h1Match = content.match(/^#\s+(.+)$/m);
                if (h1Match && !h1Match[1].includes('Consent') && !h1Match[1].includes('Cultural Review')) {
                  title = h1Match[1];
                }
              }
              ...
              if (!title) {
                title = item.name.replace('.md', '').replace(/^\d+-/, '').replace(/-/g, ' ')
        ...
        await scanDir(sectionPath);

        // Parse section name (01-identity -> Identity)
        const sectionTitle = entry.name.replace(/^\d+-/, '').replace(/-/g, ' ')
          .replace(/\b\w/g, c => c.toUpperCase());
...
app.get('/api/wiki/page', async (req, res) => {
  try {
    ...
    const fullPath = join(COMPENDIUM_PATH, pagePath);

    // Security: ensure path is within compendium
    if (!fullPath.startsWith(COMPENDIUM_PATH)) {
    ...
    const title = titleMatch ? titleMatch[1] : pagePath.split('/').pop().replace('.md', '');
...
app.get('/api/wiki/search', async (req, res) => {
  try {
    ...
    async function searchDir(dirPath, sectionId = '') {
      const items = await readdir(dirPath, { withFileTypes: true });

      for (const item of items) {
        if (item.name.startsWith('_') || item.name.startsWith('.') ||
            item.name === 'node_modules' || item.name === 'scripts') continue;

        const itemPath = join(dirPath, item.name);

        if (item.isDirectory()) {
          await searchDir(itemPath, sectionId || item.name);
        } else if (item.name.endsWith('.md')) {
          const content = await readFile(itemPath, 'utf-8');
          ...
          if (lowerContent.includes(query)) {
            ...
            const relativePath = itemPath.replace(COMPENDIUM_PATH + '/', '');
    ...
    await searchDir(COMPENDIUM_PATH);
