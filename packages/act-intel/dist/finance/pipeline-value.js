 ...
 */
export async function fetchPipelineValue(supabase, opts = {}) {
    ...
    for (const r of rows) {
        ...
        if (!byType[t])
            byType[t] = { weighted: 0, total: 0, count: 0 };
    ...
    for (const r of rows) {
        const s = r.stage;
        if (!byStage[s])
            byStage[s] = { weighted: 0, total: 0, count: 0 };
...
//# sourceMappingURL=pipeline-value.js.map