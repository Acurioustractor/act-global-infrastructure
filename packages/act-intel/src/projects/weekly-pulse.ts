...
export interface ProjectPulseEntry {
  ...
  contacts: {
    warm: Array<{ name: string; temperature: number | null }>
    cooling: Array<{ name: string; temperature: number | null }>
  }
}
...
export async function fetchWeeklyProjectPulse(
  supabase: SupabaseQueryClient,
  opts: WeeklyPulseOptions = {}
): Promise<WeeklyProjectPulseResult> {
  const now = new Date()
  ...
  if (opts.project_code) {
    ...
    for (const k of ((recentKnowledge || []) as any[])) {
      if (k.project_code && !ARCHIVED_OR_META_CODES.has(k.project_code)) codes.add(k.project_code)
    }
    for (const g of ((recentGrants || []) as any[])) {
      if (g.project_code && !ARCHIVED_OR_META_CODES.has(g.project_code)) codes.add(g.project_code)
    }
  ...
  const projects: ProjectPulseEntry[] = []

  for (const code of projectCodes) {
    // Actions
    const actions = ((allActions || []) as any[]).filter((a) => a.project_code === code)
    ...
    // Decisions
    const decisions = ((allDecisions || []) as any[]).filter((d) => d.project_code === code)

    // Last meeting
    const meetings = ((allMeetings || []) as any[]).filter((m) => m.project_code === code)
    ...
    // Grants
    const grants = ((allGrants || []) as any[]).filter((g) => g.project_code === code)
    ...
    const invoices = includeFinancials
      ? ((allInvoices || []) as any[]).filter((inv) => inv.tracking_category?.includes(code))
      : []
    ...
    const warmContacts = contacts
      .filter((c: any) => c.temperature_trend === 'warming' || (c.temperature && c.temperature >= 70))
      .slice(0, 2)
      .map((c: any) => ({ name: c.full_name, temperature: c.temperature }))
    const coolingContacts = contacts
      .filter((c: any) => c.temperature_trend === 'cooling')
      .slice(0, 2)
      .map((c: any) => ({ name: c.full_name, temperature: c.temperature }))

    // Health
    const healthEntries = ((allHealth || []) as any[]).filter((h) => h.project_code === code)
    ...
    const statusLabel: 'ACTIVE' | 'QUIET' | 'STALE' =
      daysSinceActivity !== null && daysSinceActivity <= 7 ? 'ACTIVE'
  ...
  let untaggedActionCount: number | null = null
  if (!opts.project_code) {
  ...
  return {
    ...
    summary: {
      ...
      totalOpenActions: ((allActions || []) as any[]).length,
      totalPendingDecisions: ((allDecisions || []) as any[]).length,
      totalActiveGrants: ((allGrants || []) as any[]).length,
    },
    untaggedActionCount,
  }
}
