...
import type { SupabaseQueryClient, CashflowResult, CashflowMonth } from '../types.js'
...
export async function fetchCashflow(
  ...
  for (const tx of transactions) {
    ...
    if (t.type === 'RECEIVE') monthIncome += Math.abs((t.total as number) || 0)
    else if (t.type === 'SPEND') monthExpenses += Math.abs((t.total as number) || 0)
  }
  ...
  for (const inv of invoices) {
    ...
    const amt = Math.abs((i.amount_due as number) || 0)
    if (i.type === 'ACCREC') receivables += amt
    else if (i.type === 'ACCPAY') payables += amt
  }

  // Averages from last 6 months of snapshots
  const recent = snapshots.slice(-6)
  const avgIncome = recent.length > 0 ? recent.reduce((s: number, r: unknown) => s + Number((r as Record<string, unknown>).income || 0), 0) / recent.length : 0
  const avgExpenses = recent.length > 0 ? recent.reduce((s: number, r: unknown) => s + Number((r as Record<string, unknown>).expenses || 0), 0) / recent.length : 0
  const lastBalance = snapshots.length > 0 ? Number((snapshots[snapshots.length - 1] as Record<string, unknown>).closing_balance || 0) : 0
  ...
  // History from view
  const history: CashflowMonth[] = ((cashflowViewRes.data || []) as Array<Record<string, unknown>>)
    .reverse()
    .map(m => ({
      ...
      is_projection: m.is_projection as boolean,
  ...
  return {
    ...
    projections,
    history,
    scenarios: ((scenariosRes.data || []) as Array<Record<string, unknown>>).map(s => ({
