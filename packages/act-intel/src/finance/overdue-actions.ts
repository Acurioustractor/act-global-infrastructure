...
export async function fetchOverdueActions(
  supabase: SupabaseQueryClient
): Promise<OverdueActionsResult> {
  const now = new Date()
  ...
  const actions: OverdueActionItem[] = []
  ...
  if ((untaggedCount || 0) > 0) {
    const topVendors = ((untagged || []) as any[])
  ...
  if ((overdueCount || 0) > 0) {
    const totalOverdue = ((overdueInvoices || []) as any[]).reduce(
      (s, i) => s + Math.abs(Number(i.amount_due) || 0), 0
    )
    const invoiceList = ((overdueInvoices || []) as any[])
  ...
  if ((grantCount || 0) > 0) {
    const grantList = ((passedGrants || []) as any[])
