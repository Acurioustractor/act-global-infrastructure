...
export async function fetchWeeklyFinancialReview(
  supabase: SupabaseQueryClient,
  opts: WeeklyReviewOptions = {}
): Promise<WeeklyFinancialReviewResult> {
  const now = new Date()
  ...
  const prior = (snapshots as any[])?.[1]
  ...
  for (const tx of ((weekTxns || []) as any[])) {
  ...
  const topIncome = Object.entries(incomeByContact).sort(([, a], [, b]) => b - a).slice(0, 3) as Array<[string, number]>
  const topExpenses = Object.entries(expenseByContact).sort(([, a], [, b]) => b - a).slice(0, 3) as Array<[string, number]>
  ...
  const overdueItems: WeeklyReviewOverdueInvoice[] = []
  let totalDue = 0
  ...
  // -- 5. PROJECT SPEND --
  const projectSpendMap: Record<string, number> = {}
  for (const tx of ((weekTxns || []) as any[])) {
  ...
  const projectSpend: WeeklyReviewProjectSpend[] = Object.entries(projectSpendMap)
    .sort(([, a], [, b]) => b - a)
    .slice(0, 5)
  ...
  const grantDeadlines: WeeklyReviewGrantDeadline[] = []
  ...
  const rdVendors = new Set(((rdRules || []) as any[]).map((r) => r.vendor_name))
  let weekRd = 0
  for (const tx of ((weekTxns || []) as any[])) {
  ...
  let ytdRd = 0
  for (const tx of ((ytdTxns || []) as any[])) {
  ...
  // -- 9. ACTION ITEMS --
  const actionItems: WeeklyReviewActionItem[] = []
