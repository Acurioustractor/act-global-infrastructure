...
import { fetchOverdueActions, type OverdueActionsResult } from '../finance/overdue-actions.js'
import { fetchContactsNeedingAttention, type ContactAttentionResult } from '../contacts/attention.js'
import { fetchUnansweredEmails, type UnansweredEmailsResult } from '../emails/search.js'
import { fetchReceiptPipeline } from '../finance/receipt-pipeline.js'
import { fetchOutstandingInvoices, type OutstandingInvoicesResult } from '../finance/outstanding-invoices.js'
...
export async function fetchDailyReview(
  ...
  opts: DailyReviewOptions = {}
): Promise<DailyReviewResult> {
  const pc = opts.project_code

  // Run all queries in parallel
  const [briefing, grantDeadlines, financeActions, contacts, emails, receipts, invoices] =
    await Promise.all([
  ...
  for (const a of financeActions.actions) {
    ...
    if (a.priority === 'critical') critical.push(item)
