...
import { createTask, updateTask, getTask, addComment, getComments } from './notion.js';
program
    .name('act-agent')
    ...
    try {
        ...
        while (true) {
            ...
            if (comments.results.length > initialCount) {
                ...
                const newComments = comments.results.filter((c) => c.id !== lastCommentId);
                const userComment = newComments.find((c) => c.created_by?.type === 'person' ||
                    !c.rich_text?.[0]?.text?.content?.startsWith('🤖'));
                if (userComment) {
                    const text = userComment.rich_text
