...
export async function getTask(urlOrId) {
    const pageId = extractPageId(urlOrId);
    return notion.pages.retrieve({ page_id: pageId });
}
// Update task properties
export async function updateTask(urlOrId, properties) {
    const pageId = extractPageId(urlOrId);
...
export async function getComments(urlOrId) {
    const pageId = extractPageId(urlOrId);
    return notion.comments.list({ block_id: pageId });
}
// Add a comment to a task
export async function addComment(urlOrId, text) {
    const pageId = extractPageId(urlOrId);
